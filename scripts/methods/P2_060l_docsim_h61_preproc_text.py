#!/usr/bin/env python3
"""P2_060l — D60.l: H6.1 preprocessing applied before DocSim head for variant=text only.

Identical to P2_060j EXCEPT: when computing the DocSim cosine for variant=text,
inputs are first passed through H6.1 preproc (grayscale -> SHARPEN -> autocontrast
-> 3-channel RGB) before the frozen CLIP+DINOv2 encoder pair. Other 4 variants
byte-equal to D60.j (raw RGB into DocSim for {all, all_no_mask}, H4.e fusion for
formula, H6.1 + CLIP-only for table).

Hypothesis: D60.e showed H6.1-on-DocSim was a near-noise tie for {all,all_no_mask}
(post-fix 0.4052 vs anchor 0.4042). variant=text differs structurally — its pages
are dominated by clean line-text content that H6.1 was originally tuned to enhance
(grayscale sharpening lifts per-cell glyph contrast). If H6.1 on DocSim text
encoder lifts text DocSim cc above 0.4139 (the D60.j confirmed value), spearman_mean
rises by (text_lift)/5.

Predicted spearman_mean: 0.4093 (tie if text DocSim is preproc-invariant) to
0.414 (if H6.1 lifts text DocSim cc to 0.43-0.44). Most-likely outcome ~0.411,
within 0.4093-0.413 band. KEEP if >= 0.412 (~+0.003 over D60.j post-fix anchor
0.4093). Kill switch: text DocSim cc <= 0.40 (H6.1 destabilizes the LoRA head's
training distribution for text — close preproc-on-DocSim class).

Control: D60.j (P2_060j_docsim_text_all_anomask, 0.4093, post-fix scale).
"""
...
METHOD_ID = "P2_060l_docsim_h61_preproc_text"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
# Strategy flags (same as P2_060j)
...
USE_PREPROC = variant in ("table", "all_no_mask")   # H6.1 preproc gate for non-DocSim cc paths
USE_DOCSIM = variant in ("text", "all", "all_no_mask")
# NEW for D60.l: H6.1 preproc applied before DocSim head, variant=text only.
USE_PREPROC_DOCSIM = variant == "text"
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_clip_only=%s use_preproc=%s use_docsim=%s use_preproc_docsim=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_CLIP_ONLY, USE_PREPROC, USE_DOCSIM, USE_PREPROC_DOCSIM,
)
...
def _docsim_cosine_batch(orig_pils, recon_pils):
    ...
    For variant=text, inputs are first passed through H6.1 preproc
    (grayscale -> SHARPEN -> autocontrast -> RGB). Other DocSim variants
    use raw RGB (matching how triplets were built).
    """
    def _encode(pils):
        if USE_PREPROC_DOCSIM:
            preprocessed = [_baseline_clip_preprocess(p) for p in pils]
        else:
            preprocessed = [p.convert("RGB") for p in pils]
        clip_batch = torch.stack([_clip_preprocess(p) for p in preprocessed]).to(device)
        dino_batch = torch.stack([_dino_transform(p) for p in preprocessed]).to(device)
        with torch.no_grad():
