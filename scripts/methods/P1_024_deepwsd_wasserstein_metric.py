#!/usr/bin/env python3
"""P1_024_deepwsd_wasserstein_metric: D24 / H_DeepWSD — msswd as 4th term REPLACING DISTS for variant=all.

Hypothesis: Replacing DISTS with a Wasserstein-family metric (msswd — Multi-Scale
Sliced Wasserstein Distance, pyiqa "msswd", lower_better=True, FR) as the 4th term
in variant=all multi_composite will change spearman_mean vs H11.2 (0.3600).

DeepWSD was specified in D24 but is NOT available in pyiqa. msswd is the only
Wasserstein-family metric in pyiqa and is the closest substitute. Scores are
normalised to [0,1] via division by 10 (pyiqa score_range=[0,~10]) before
insertion into the composite formula as (1 - msswd_normed).
...
METHOD_ID = "P1_024_deepwsd_wasserstein_metric"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
# msswd score_range is [0, ~10]; normalise to [0, 1] before 1-x transformation
MSSWD_NORM_FACTOR = 10.0
...
USE_MSSWD = variant == "all"          # msswd-augmented composite only for 'all' variant
...
log.info(
    "device=%s variant=%s method=%s use_msswd=%s use_clip_only=%s use_preproc=%s",
    device, variant, METHOD_ID, USE_MSSWD, USE_CLIP_ONLY, USE_PREPROC,
)

_preprocessor = ImagePreprocessor()

# ── msswd setup via pyiqa (only for variant=all) ─────────────────────────────
if USE_MSSWD:
    import pyiqa
    log.info("Loading pyiqa msswd metric (variant=all only)...")
    _msswd_metric = pyiqa.create_metric("msswd", as_loss=False).to(device)
    # msswd accepts standard [0,1] RGB tensors; resize to 256x256 for consistency
    _msswd_transform = T.Compose([
        T.Resize((256, 256), interpolation=T.InterpolationMode.BICUBIC),
        T.ToTensor(),  # [0,1]
    ])
else:
    log.info("Skipping msswd load (variant=%s: original composite formula)", variant)
    _msswd_metric = None
    _msswd_transform = None


def _msswd_score(orig_pil: Image.Image, recon_pil: Image.Image) -> float:
    """Compute msswd distance (lower=more similar), normalised to [0,1]."""
    ...
    # Normalise by MSSWD_NORM_FACTOR and clip to [0, 1]
    return float(np.clip(dist.item() / MSSWD_NORM_FACTOR, 0.0, 1.0))
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_MSSWD:
        # variant=all: msswd-augmented rebalanced composite (replaces DISTS slot)
        msswd_normed = _msswd_score(orig_pil, recon_pil)
        composite = (
            0.3 * ssim_val
            + 0.2 * max(0.0, 1.0 - mse_val)
            + 0.2 * max(0.0, 1.0 - lpips_val)
            + 0.3 * max(0.0, 1.0 - msswd_normed)
        )
        multi_metric = {
            "ssim": ssim_val,
            "mse": mse_val,
            "lpips": lpips_val,
            "msswd_normed": msswd_normed,
            "composite": composite,
        }
    else:
