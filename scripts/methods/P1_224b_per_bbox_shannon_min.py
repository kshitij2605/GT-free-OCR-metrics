#!/usr/bin/env python3
"""P1_224b — D224.b: per-bbox-element MIN of mean-Shannon-confidence.

Replaces D215.l's rolling-K-token window-MIN entropy aggregation with
element-aligned aggregation: for each Qwen-OCR HTML element identified
by `data-bbox="..."` in the token stream, take the mean Shannon-
confidence over tokens within that element. Aggregate per-page via MIN
across elements. Mirrors D60.m's per-cell-MEAN DocSim breakthrough.
...
METHOD_ID = "P1_224b_per_bbox_shannon_min"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
def _per_bbox_min_shannon_confidence(page_name):
    """Per-bbox-element MIN of mean Shannon-confidence.

    Parses the joined token stream for `data-bbox="` markers; partitions
    tokens by the most recent bbox marker. For each partition (element),
    computes the MEAN of per-token Shannon-confidence
    (1 - H/log(K_top) over the renormalized top_logprobs distribution).
    Returns MIN across elements. Tokens BEFORE the first bbox marker
    are grouped as a leading element (typically html/body header).

    Returns 0.5 (neutral) if logprobs missing or no usable confidences.
    """
    ...
    # Per-token Shannon-confidence (None where unavailable)
    confs = []
    for t in tokens:
    ...
    # Element segmentation: find char positions of `data-bbox="` then map to token indices.
    text_chars = []  # cumulative char position at start of each token
    pos = 0
    for t in tokens:
        text_chars.append(pos)
        pos += len(t["token"])
    full = "".join(t["token"] for t in tokens)
    bbox_char_positions = [m.start() for m in _BBOX_RE.finditer(full)]
    if not bbox_char_positions:
        # No structured elements — fall back to global mean (page-level)
        valid = [c for c in confs if c is not None]
        return float(np.mean(valid)) if valid else 0.5

    # Map char positions → token indices (first token whose start >= char pos)
    token_starts = np.array(text_chars)
    bbox_token_indices = []
    for cp in bbox_char_positions:
        idx = int(np.searchsorted(token_starts, cp, side="right") - 1)
        if idx < 0:
            idx = 0
        bbox_token_indices.append(idx)

    # Build element ranges: [bbox_token_indices[i], bbox_token_indices[i+1]),
    # plus a leading segment [0, bbox_token_indices[0]) if it has confs.
    element_ranges = []
    if bbox_token_indices[0] > 0:
        element_ranges.append((0, bbox_token_indices[0]))
    for i, start in enumerate(bbox_token_indices):
        end = bbox_token_indices[i + 1] if i + 1 < len(bbox_token_indices) else len(tokens)
        element_ranges.append((start, end))

    element_means = []
    for s, e in element_ranges:
        seg = [c for c in confs[s:e] if c is not None]
        if seg:
            element_means.append(float(np.mean(seg)))
    if not element_means:
        valid = [c for c in confs if c is not None]
        return float(np.mean(valid)) if valid else 0.5
    return float(np.min(element_means))


def _fuse(prod_cc, page_name):
    conf = _per_bbox_min_shannon_confidence(page_name)
...
log.info("Done. %d pages -> %s (alpha=%.2f, source=per_bbox_shannon_min)",
         len(results), out_path, ALPHA_ENTROPY)
