...
METHOD_ID = "P1_149_elem_docsim_p10"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
def _content_element_clip_p10(orig_pil, recon_pil, page_dir):
    """DocSim cosine P10 over all content-element bboxes (text+formula+table).

    D149: uses _docsim_cosine_batch (fine-tuned CLIP+DINOv2 LoRA) instead of
    raw CLIP, to test whether document-specific features give better element-level
    signal for all/all_no_mask.
    ...
    for s in range(0, len(orig_crops), BATCH_SIZE):
        sims.extend(_docsim_cosine_batch(orig_crops[s:s + BATCH_SIZE], recon_crops[s:s + BATCH_SIZE]))
