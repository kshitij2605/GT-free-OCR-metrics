...
METHOD_ID = "P1_224d_variant_conditional_source"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
SOURCE_ROUTING = {
    ...
    "table":       "window_min_mean_shannon",  # alpha=0, source unused
    "all":         "window_min_mean_shannon",
    "all_no_mask": "window_min_mean_shannon",
}
...
def _fuse(prod_cc, page_name):
    """Convex combo of production cc and entropy confidence at ALPHA_ENTROPY.

    Entropy SOURCE per-variant routed via SOURCE_ROUTING:
      - text: class-filtered per-bbox MIN over shannon_entropy_max (D224.c)
      - formula/all/all_no_mask: window-MIN of mean-Shannon-confidence (D215.l)
      - table: alpha=0 so source is moot.
    """
