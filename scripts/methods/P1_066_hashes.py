#!/usr/bin/env python3
"""P1_066: pHash + wHash + dHash composite (D66).

Replaces the SSIM slot in multi_composite with an average of 3 perceptual
hashes (pHash, wHash, dHash) at 16x16 (256-bit) resolution. Similarity =
1 - normalized Hamming distance. CPU-only and very fast.

MSE and LPIPS are kept on binarized images per baseline convention; CLIP
cosine is kept intact. Composite = 0.4*hash_sim + 0.3*(1-mse) + 0.3*(1-lpips).

Reference: priorities.md #66; Zauner 2010 (pHash).
"""
...
def _hash_composite(orig_pil: Image.Image, recon_pil: Image.Image) -> float:
    """Average similarity across pHash, wHash, dHash at 16x16."""
    orig_g = orig_pil.convert("L")
    recon_g = recon_pil.convert("L")
    ph_d = (imagehash.phash(orig_g, hash_size=HASH_SIZE) - imagehash.phash(recon_g, hash_size=HASH_SIZE)) / HASH_BITS
    wh_d = (imagehash.whash(orig_g, hash_size=HASH_SIZE) - imagehash.whash(recon_g, hash_size=HASH_SIZE)) / HASH_BITS
    dh_d = (imagehash.dhash(orig_g, hash_size=HASH_SIZE) - imagehash.dhash(recon_g, hash_size=HASH_SIZE)) / HASH_BITS
    sim = 1.0 - (0.4 * ph_d + 0.3 * wh_d + 0.3 * dh_d)
...
for i, page_dir in enumerate(page_dirs):
    ...
    hash_sim = _hash_composite(orig_pil, recon_pil)
    mse_score = _mse(orig_pil, recon_pil)
    lpips_score = _lpips_score(orig_pil, recon_pil)
    composite = 0.4 * hash_sim + 0.3 * max(0.0, 1.0 - mse_score) + 0.3 * max(0.0, 1.0 - lpips_score)
    ...
    meta = {
        ...
        "plain_text_length": 0,
        "multi_metric": {
            "ssim": hash_sim,   # slot reused for hash composite similarity
            "mse": mse_score,
            "lpips": lpips_score,
            "composite": composite,
        },
