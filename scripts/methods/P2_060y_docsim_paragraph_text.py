#!/usr/bin/env python3
"""P2_060y — D60.y: paragraph-clustered per-bbox MIN DocSim for variant=text.

Distinct from D60.n (REVERTED) which used raw text-line bboxes from
ocr_elements.json. D60.y clusters those text-line bboxes into paragraph
groups via vertical-proximity + x-overlap heuristics, producing fewer,
larger, internally homogeneous bboxes that better match the page-level
supervised DocSim head's training distribution.

Clustering: sort text-line bboxes by y_center; group consecutive lines
where (vertical gap <= 1.6 * median_line_height) AND (x-overlap >= 50%);
paragraph bbox = union of cluster.

For variant=text: per-paragraph DocSim cosine, MIN-aggregated. Empty/
no-paragraph pages fall back to page-level DocSim cc (preserves D60.j).

Other 4 variants byte-equal to D60.p:
  - table: per-cell MIN DocSim (H6.1 CLIP-only fallback)
  - formula: H4.e (50/50 CLIP + DINOv2 fusion)
  - all, all_no_mask: page-level DocSim cc

Predicted spearman_mean: 0.443-0.452, most likely 0.446 (+0.0007 vs
D60.p 0.4453). KEEP if >= 0.4475. Kill switch: text cc <= 0.40.
"""
...
# Paragraph clustering thresholds
...
# Strategy flags (mirror of D60.p with text-bbox added)
USE_DISTS = variant == "all"
USE_PREPROC = variant in ("table", "all_no_mask")  # H6.1 preproc gate
USE_DOCSIM_PAGE = variant in ("all", "all_no_mask")  # page-level DocSim
USE_DOCSIM_BBOX_TABLE = variant == "table"           # per-cell MIN (D60.p)
USE_DOCSIM_BBOX_TEXT = variant == "text"             # per-paragraph MIN (D60.y new)
TEXT_FALLBACK_PAGE_DOCSIM = variant == "text"        # text empty-paragraph fallback
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_preproc=%s use_docsim_page=%s use_docsim_bbox_table=%s use_docsim_bbox_text=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_PREPROC, USE_DOCSIM_PAGE,
    USE_DOCSIM_BBOX_TABLE, USE_DOCSIM_BBOX_TEXT,
)
...
def _cluster_lines_into_paragraphs(line_bboxes: list) -> list:
    """Cluster text-line bboxes into paragraph bboxes.

    Heuristic: sort by y_center; greedily merge consecutive lines into a
    paragraph if vertical gap <= PARA_GAP_LINE_HEIGHT_RATIO * median_line_height
    AND x-overlap >= PARA_X_OVERLAP_FRAC of the shorter line span. Each
    paragraph bbox is the union (envelope) of its cluster's line bboxes.

    Returns a list of paragraph bboxes [x1, y1, x2, y2].
    """
    valid = []
    ...
    if not valid:
        return []
    # median line height as gap threshold base
    heights = [b[3] - b[1] for b in valid]
    median_h = float(np.median(heights))
    gap_thresh = PARA_GAP_LINE_HEIGHT_RATIO * median_h

    valid.sort(key=lambda b: (b[1] + b[3]) / 2.0)
    ...
    for b in valid[1:]:
        ...
        v_gap = max(0.0, cy1 - prev_y2)
        # x-overlap as fraction of shorter span
        ox = max(0.0, min(prev_x2, cx2) - max(prev_x1, cx1))
        shorter = min(prev_x2 - prev_x1, cx2 - cx1)
        x_overlap_frac = ox / shorter if shorter > 0 else 0.0
        if v_gap <= gap_thresh and x_overlap_frac >= PARA_X_OVERLAP_FRAC:
            cur.append(b)
        else:
            clusters.append(cur)
            cur = [b]
    ...
    paragraphs = []
    for cl in clusters:
        x1 = min(b[0] for b in cl)
        y1 = min(b[1] for b in cl)
        x2 = max(b[2] for b in cl)
        y2 = max(b[3] for b in cl)
        paragraphs.append([x1, y1, x2, y2])
    return paragraphs


def _docsim_per_bbox_min_score(orig_pil: Image.Image, recon_pil: Image.Image, bboxes) -> float | None:
    """MIN DocSim cosine over per-bbox crops. None if no usable crops."""
    orig_crops = []
    recon_crops = []
    for bb in bboxes:
...
def _flush_batch():
    ...
    if USE_DOCSIM_PAGE or (USE_DOCSIM_BBOX_TEXT and TEXT_FALLBACK_PAGE_DOCSIM):
        # variant in {all, all_no_mask}: page-level DocSim.
        # variant=text empty-paragraph pages: page-level DocSim fallback.
        ...
        # variant=table empty-bbox fallback: H6.1 CLIP-only.
        ...
        # variant=formula: H4.e (50/50 CLIP + DINOv2 fusion).
        clip_sims = _clip_cosine_batch(orig_pils_buf, recon_pils_buf)
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DOCSIM_BBOX_TEXT:
        # variant=text: cluster ocr_elements.json text-line bboxes into paragraphs.
        bbox_path = page_dir / "ocr_elements.json"
        line_bboxes = []
        if bbox_path.exists():
            try:
                with bbox_path.open("r") as f:
                    elems = json.load(f)
                line_bboxes = [e.get("bbox") for e in elems if e.get("bbox")]
            except Exception as e:
                log.warning("page=%s ocr_elements load failed: %s", page_dir.name, e)
                line_bboxes = []
        paragraphs = _cluster_lines_into_paragraphs(line_bboxes) if line_bboxes else []
        score = _docsim_per_bbox_min_score(orig_pil, recon_pil, paragraphs) if paragraphs else None
        if score is not None:
            ...
            # Empty/no-paragraph page: page-level DocSim fallback.
            n_fallback_pages += 1
            orig_pils_buf.append(orig_pil)
            recon_pils_buf.append(recon_pil)
            meta_buf.append(meta)
    elif USE_DOCSIM_BBOX_TABLE:
        # variant=table: per-cell MIN (D60.p semantics).
        ...
        if bbox_path.exists():
            try:
                ...
                log.warning("page=%s table bbox load failed: %s", page_dir.name, e)
                bboxes = []
        score = _docsim_per_bbox_min_score(orig_pil, recon_pil, bboxes) if bboxes else None
        if score is not None:
            n_bbox_pages += 1
            meta["clip_compare"] = {"clip_cosine": float(score)}
            results.append(meta)
...
if USE_DOCSIM_BBOX_TEXT:
    avg_paras = n_paragraphs_total / max(1, n_bbox_pages)
    log.info("variant=text paragraph-path pages: %d (avg %.1f paragraphs/page) / fallback pages: %d",
             n_bbox_pages, avg_paras, n_fallback_pages)
elif USE_DOCSIM_BBOX_TABLE:
