#!/usr/bin/env python3
"""P1_166_siglip_table_clip: H12.1 — SigLIP-base for variant=table CLIP path only (Branch 12 root).

H12.1 hypothesis: SigLIP (Zhai et al. 2023, ICCV) uses sigmoid contrastive loss instead of
softmax + WebLI 4B image-text pairs. Its CLS token captures spatial structure differently than
OpenCLIP. Replacing OpenCLIP with SigLIP-base-patch16-224 for the variant=table CLIP path ONLY
-- while keeping H6.1 baseline preprocessing (grayscale+SHARPEN+autocontrast->RGB) -- could
break the table cc=0.277 ceiling that Branch 6 (OpenCLIP encoder/resolution swaps) exhausted.

Branch 6 results for context:
  H6.1 (baseline preproc): table cc=0.277  <- control
  H6.2 (ViT-L/14):         table cc=0.250  <- worse
  H6.4 (ViT-B/32@336):     table cc=0.252  <- worse
...
clip_cosine logic:
- variant=table: SigLIP-base cosine ONLY, with H6.1 baseline preprocessing applied
                 (grayscale -> SHARPEN -> autocontrast -> RGB) before SigLIP processor
...
METHOD_ID = "P1_166_siglip_table_clip"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
USE_SIGLIP = variant == "table"       # H12.1: SigLIP for table variant only
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_clip_only=%s use_table_preproc=%s use_siglip=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_CLIP_ONLY, USE_TABLE_PREPROC, USE_SIGLIP,
)
...
# ── SigLIP setup (only for variant=table) ─────────────────────────────────────
if USE_SIGLIP:
    log.info("Loading SigLIP-base-patch16-224 (H12.1: table variant only)...")
    _siglip_model = AutoModel.from_pretrained("google/siglip-base-patch16-224").eval().to(device)
    _siglip_processor = AutoProcessor.from_pretrained("google/siglip-base-patch16-224")
    _clip_model = None
    _clip_preprocess = None
else:
    # ── OpenCLIP setup (only for non-table variants) ───────────────────────────
...
# ── SigLIP cosine (batched, for variant=table only) ───────────────────────────

def _siglip_cosine_batch(orig_pils: list, recon_pils: list) -> list:
    def _enc(pils):
        # H6.1-style baseline preprocessing first, then SigLIP processor
        processed = [_baseline_clip_preprocess(p) for p in pils]
        inputs = _siglip_processor(images=processed, return_tensors="pt").to(device)
        with torch.no_grad():
...
# ── OpenCLIP cosine (batched, for non-table variants) ─────────────────────────
...
def _flush_batch():
    ...
    if not orig_pils_buf:
        return
    if USE_SIGLIP:
        # H12.1: table variant uses SigLIP cosine alone, with H6.1 baseline preprocessing
        clip_sims = _siglip_cosine_batch(orig_pils_buf, recon_pils_buf)
        for meta, clip_cos in zip(meta_buf, clip_sims):
            meta["clip_compare"] = {"clip_cosine": float(clip_cos)}
            results.append(meta)
    elif USE_CLIP_ONLY:
        # Should not reach here (USE_SIGLIP == USE_CLIP_ONLY for table), kept for safety
