#!/usr/bin/env python3
"""D68 — spatially-aware grid-hash MIN supplement.

Divides each page into a GRID_ROWS x GRID_COLS grid (default 4x4 = 16 cells).
For each grid cell, computes pHash similarity (imagehash, hash_size=8 → 64-bit).
Aggregates with MIN → the worst-rendered spatial region.

Uses this as an additional β_grid supplement for all/all_no_mask variants
on top of D107b (β_table=0.20). Hypothesis: spatially localized hash captures
layout differences and localized rendering degradation orthogonal to page-level
DocSim. Unlike per-semantic-element MIN (D60.n k40 dead end), uniform grid
does not require element detection and avoids off-distribution crops.

  all: (1-0.30-0.20-β_grid)*DocSim + 0.30*entropy + 0.20*table_cell + β_grid*grid_hash_MIN
  all_no_mask: (1-0.40-0.20-β_grid)*DocSim + 0.40*entropy + 0.20*table_cell + β_grid*grid_hash_MIN

β_grid=0.05 (small supplement to preserve DocSim dominance).
KEEP if > 0.4820 (D107b best). REVERT → grid-hash supplement closed.
...
METHOD_ID = "P1_068_spatially_aware_hash_grid_min"
GRID_ROWS = 4
GRID_COLS = 4
HASH_SIZE = 8  # 64-bit pHash per cell
...
# D68: grid hash MIN supplement for all/all_no_mask
# all: (1-0.30-0.20-0.05)*DocSim + 0.30*entropy + 0.20*table_cell + 0.05*grid_hash
# all_no_mask: (1-0.40-0.20-0.05)*DocSim + 0.40*entropy + 0.20*table_cell + 0.05*grid_hash
BETA_GRID_HASH = {
    "table": 0.0,
    "all": 0.05,
    "text": 0.0,
...
def _grid_hash_min_score(orig_pil, recon_pil):
    """D68: MIN pHash similarity over GRID_ROWS x GRID_COLS spatial grid."""
    W, H = orig_pil.size
    cell_w = W // GRID_COLS
    cell_h = H // GRID_ROWS
    ...
    for r in range(GRID_ROWS):
        for c in range(GRID_COLS):
            ...
            x2 = min((c + 1) * cell_w, W)
            y2 = min((r + 1) * cell_h, H)
            oc = orig_g.crop((x1, y1, x2, y2))
            rc = recon_g.crop((x1, y1, x2, y2))
            h1 = imagehash.phash(oc, hash_size=HASH_SIZE)
            h2 = imagehash.phash(rc, hash_size=HASH_SIZE)
            ham = h1 - h2
            sims.append(1.0 - ham / HASH_BITS)
...
def _fuse(prod_cc, page_name, diqa_sim=0.0, table_cell_score=0.0, grid_hash_score=0.0):
    ...
    For variant=all/all_no_mask with BETA_TABLE_CELL>0 and/or BETA_GRID_HASH>0:
        clip_cosine = (1-alpha-beta_t-beta_g)*prod_cc + alpha*conf + beta_t*table_cell + beta_g*grid_hash
    Otherwise:
    ...
    if BETA_TABLE_CELL > 0.0 or BETA_GRID_HASH > 0.0:
        beta_t = BETA_TABLE_CELL if table_cell_score > 0.0 else 0.0
        beta_g = BETA_GRID_HASH  # grid hash always applies (all pages have grid)
        if beta_t > 0.0 or beta_g > 0.0:
            return (1.0 - ALPHA_ENTROPY - beta_t - beta_g) * prod_cc + ALPHA_ENTROPY * conf + beta_t * table_cell_score + beta_g * grid_hash_score
...
results = []
orig_pils_buf, recon_pils_buf, meta_buf, name_buf, table_cell_score_buf, grid_hash_score_buf = [], [], [], [], [], []


def _flush_batch():
    global orig_pils_buf, recon_pils_buf, meta_buf, name_buf, table_cell_score_buf, grid_hash_score_buf
    if not orig_pils_buf:
        return
    if USE_DOCSIM_PAGE:
        cos_sims = _docsim_cosine_batch(orig_pils_buf, recon_pils_buf)
        for meta, cs, name, tcs, ghs in zip(meta_buf, cos_sims, name_buf, table_cell_score_buf, grid_hash_score_buf):
            meta["clip_compare"] = {"clip_cosine": float(_fuse(float(cs), name, table_cell_score=tcs, grid_hash_score=ghs))}
            results.append(meta)
    elif variant == "table":
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DOCSIM_BBOX:
        ...
        if score is not None:
            ...
            grid_hash_score_buf.append(0.0)
        ...
        ghs = 0.0
        if BETA_GRID_HASH > 0.0:
            ghs = _grid_hash_min_score(orig_pil, recon_pil)
        grid_hash_score_buf.append(ghs)
        orig_pils_buf.append(orig_pil)
        recon_pils_buf.append(recon_pil)
        meta_buf.append(meta)
        name_buf.append(page_dir.name)
