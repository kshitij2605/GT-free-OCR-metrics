#!/usr/bin/env python3
"""P2_060m — D60.m: per-table-bbox DocSim cosine for variant=table.

Extends D60.j by changing variant=table from full-page CLIP-only (H6.1
preproc) to per-table-bbox DocSim cosine, mean-aggregated across cells.
For each page, load ocr_table_elements.json, crop both masked_original.png
and reconstructed.png to each table bbox, run DocSim LoRA head on each
crop pair, mean. Pages with no table bboxes fall back to the D60.j
baseline path (H6.1 CLIP-only) so they are not zeroed.

Other 4 variants (text, formula, all, all_no_mask) byte-equal to D60.j.

Predicted spearman_mean: 0.4112 (+0.002 vs D60.j 0.4093). KEEP if >=
0.412. Kill switch: table cc <= 0.29.
"""
...
METHOD_ID = "P2_060x_docsim_h4e_5050_formula"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
USE_PREPROC = variant in ("table", "all_no_mask")  # H6.1 preproc gate (table fallback + all_no_mask DINOv2)
USE_DOCSIM_PAGE = variant in ("text", "all", "all_no_mask")  # page-level DocSim (D60.j set)
USE_DOCSIM_BBOX = variant == "table"                          # per-table-bbox DocSim (D60.m new)
USE_DOCSIM_H4E_FUSE = variant == "formula"                    # D60.x — 50/50 fuse DocSim with H4.e for formula
NEED_DOCSIM = USE_DOCSIM_PAGE or USE_DOCSIM_BBOX or USE_DOCSIM_H4E_FUSE
NEED_DINO = variant != "table" or USE_DOCSIM_BBOX  # also need DINO for DocSim head when bbox path active
...
def _flush_batch():
    ...
    if USE_DOCSIM_PAGE:
        ...
        # Fallback path: pages without table bboxes use H6.1 CLIP-only.
        ...
        # variant=formula: D60.x — 50/50 fusion of H4.e (50/50 CLIP+DINOv2) with DocSim cc.
        clip_sims = _clip_cosine_batch(orig_pils_buf, recon_pils_buf)
        dino_sims = _dinov2_cosine_batch(orig_pils_buf, recon_pils_buf)
        docsim_sims = _docsim_cosine_batch(orig_pils_buf, recon_pils_buf)
        for meta, dino_cos, clip_cos, doc_cos in zip(meta_buf, dino_sims, clip_sims, docsim_sims):
            h4e_cos = 0.5 * dino_cos + 0.5 * clip_cos
            avg_cos = 0.5 * h4e_cos + 0.5 * doc_cos
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DOCSIM_BBOX:
        # Per-table-bbox DocSim path. Process each page individually.
