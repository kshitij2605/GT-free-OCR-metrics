...
METHOD_ID = "P1_106m_iq_fusion_all_05"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
# D106.m — gamma for D106 IQ-feature term
GAMMA_DIQA = {
    "table": 0.1,         # D106.d kept: 0.9 prod_cc + 0.1 D106
    "all": 0.05,          # D106.m: first IQ test; weights = 0.65 prod_cc + 0.30 entropy + 0.05 D106
    "text": 0.0,
    "formula": 0.5,       # D106.h peak: 0.2 prod_cc + 0.3 entropy + 0.5 D106
    "all_no_mask": 0.05,  # D106.m: first IQ test; weights = 0.55 prod_cc + 0.40 entropy + 0.05 D106
}[variant]
DIQA_EPS = 1e-6
...
def _flush_batch():
    ...
    if USE_DOCSIM_PAGE:
        ...
        for meta, cs, name, orig_p, recon_p in zip(meta_buf, cos_sims, name_buf, orig_pils_buf, recon_pils_buf):
            diqa_sim = _diqa_similarity(orig_p, recon_p) if GAMMA_DIQA > 0.0 else 0.0
            meta["clip_compare"] = {"clip_cosine": float(_fuse(float(cs), name, diqa_sim))}
            results.append(meta)
    elif variant == "table":
