#!/usr/bin/env python3
"""P1_120c4_table_clip_large: H6.2 — OpenCLIP ViT-L/14 for variant=table only.

H6.2 hypothesis: ViT-L/14 has 14-pixel patches (~256 tokens at 224x224) vs B/32's ~50 tokens —
much finer spatial resolution should capture table cell-grid structure better, raising table cc
from 0.277 → 0.29-0.32.

Isolation strategy: This script is launched as a separate process per variant by gpu_dispatch.sh.
Each process handles exactly one variant, so variant-conditional model loading is safe and clean.
For variant=table: load ViT-L-14 (laion2b_s32b_b82k) — larger, finer-grained encoder.
For all other variants: load ViT-B-32 (laion2b_s34b_b79k) — same as H6.1, no change.
...
clip_cosine logic:
- variant=table: ViT-L/14 cosine ONLY, with H6.1 baseline preprocessing applied before _clip_preprocess
...
METHOD_ID = "P1_120c4_table_clip_large"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
# H6.2: use ViT-L/14 for table, ViT-B/32 for all others
USE_LARGE_CLIP = variant == "table"
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_clip_only=%s use_table_preproc=%s use_large_clip=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_CLIP_ONLY, USE_TABLE_PREPROC, USE_LARGE_CLIP,
)
...
# ── OpenCLIP setup: variant-conditional model selection (H6.2 isolation) ─────
# This script runs as one process per variant (gpu_dispatch.sh launches per variant),
# so conditional loading here is safe — each process only ever sees one variant.
if USE_LARGE_CLIP:
    # H6.2: ViT-L/14 for table — finer 14-pixel patches (~256 tokens) capture cell-grid structure
    log.info("Loading OpenCLIP ViT-L-14 (laion2b_s32b_b82k) for variant=table...")
    _clip_model, _, _clip_preprocess = open_clip.create_model_and_transforms(
        "ViT-L-14", pretrained="laion2b_s32b_b82k"
    )
else:
    # All other variants: keep ViT-B-32 (unchanged from H6.1)
    log.info("Loading OpenCLIP ViT-B-32 (laion2b_s34b_b79k) for variant=%s...", variant)
    _clip_model, _, _clip_preprocess = open_clip.create_model_and_transforms(
...
def _flush_batch():
    ...
    if USE_CLIP_ONLY:
        # table variant: use CLIP cosine alone (H4.e strategy), with H6.1 preprocessing + H6.2 ViT-L/14
