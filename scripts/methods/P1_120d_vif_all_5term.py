#!/usr/bin/env python3
"""P1_120d_vif_all_5term: H10.1 — VIF as 5th term in mc for variant=all only.

H10.1 hypothesis: Adding VIF (Visual Information Fidelity, Sheikh & Bovik 2006) as a 5th
term in the multi_composite formula for variant=all only will lift mc above 0.2834 by
providing orthogonal information-theoretic signal that DISTS texture statistics miss.

multi_composite formulas:
  variant=all (USE_DISTS=True, USE_VIF=True):
    composite = 0.25*SSIM + 0.15*(1-MSE) + 0.15*(1-LPIPS) + 0.25*(1-DISTS) + 0.20*VIF
  all other variants (USE_DISTS=False, USE_VIF=False):
...
METHOD_ID = "P1_120d_vif_all_5term"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
USE_VIF = variant == "all"            # VIF 5th-term only for 'all' variant
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_vif=%s use_clip_only=%s use_table_preproc=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_VIF, USE_CLIP_ONLY, USE_TABLE_PREPROC,
)

_preprocessor = ImagePreprocessor()

# ── DISTS + VIF setup via pyiqa (only for variant=all) ───────────────────────
if USE_DISTS:
    import pyiqa
...
if USE_VIF:
    import pyiqa
    log.info("Loading pyiqa VIF metric (variant=all only)...")
    # Try vif first; fall back to vifs (VIF-spatial) if not registered
    try:
        _vif_metric = pyiqa.create_metric("vif", as_loss=False).to(device)
        log.info("VIF metric loaded successfully (key='vif')")
    except Exception as e_vif:
        log.warning("pyiqa 'vif' failed (%s); trying 'vifs'...", e_vif)
        try:
            _vif_metric = pyiqa.create_metric("vifs", as_loss=False).to(device)
            log.info("VIF metric loaded successfully (key='vifs')")
        except Exception as e_vifs:
            log.error("Both 'vif' and 'vifs' failed (%s). Falling back to piq.VIFLoss.", e_vifs)
            import piq
            _vif_metric = None
            _use_piq_vif = True
    else:
        _use_piq_vif = False
    # Reuse same 256x256 BICUBIC transform as DISTS
    _vif_transform = _dists_transform
else:
    _vif_metric = None
    _vif_transform = None
    _use_piq_vif = False
...
def _vif_score(orig_pil: Image.Image, recon_pil: Image.Image) -> float:
    """Compute VIF similarity (higher=better). Returns clipped to [0,1]."""
    orig_t = _vif_transform(orig_pil.convert("RGB")).unsqueeze(0).to(device)
    recon_t = _vif_transform(recon_pil.convert("RGB")).unsqueeze(0).to(device)
    with torch.no_grad():
        if _use_piq_vif:
            import piq
            v = 1.0 - piq.VIFLoss(reduction="mean")(orig_t, recon_t)
        else:
            v = _vif_metric(orig_t, recon_t)
    return float(np.clip(v.item() if hasattr(v, "item") else float(v), 0.0, 1.0))
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DISTS:
        # variant=all: DISTS+VIF 5-term composite (H10.1)
        dists_val = _dists_score(orig_pil, recon_pil)
        vif_val = _vif_score(orig_pil, recon_pil)
        composite = (
            0.25 * ssim_val
            + 0.15 * max(0.0, 1.0 - mse_val)
            + 0.15 * max(0.0, 1.0 - lpips_val)
            + 0.25 * max(0.0, 1.0 - dists_val)
            + 0.20 * vif_val   # VIF already higher=better in [0,1]
        )
        multi_metric = {
            "ssim": ssim_val,
            "mse": mse_val,
