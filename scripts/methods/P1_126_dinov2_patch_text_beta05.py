#!/usr/bin/env python3
"""D122 — SSIM text supplement beta=0.15 (text β sweep step 3).

D120 confirmed text SSIM at β=0.05 lifts text Spearman 0.4640→0.4710 (+0.0070, mean 0.4891 NEW BEST).
D121 β=0.10 continued: text 0.4710→0.4769 (+0.0059, mean 0.4903 NEW BEST).
D122 β=0.15 tests whether text β-axis continues rising (diminishing returns trend noted).

  text: (1-0.40-0.15)*DocSim + 0.40*entropy + 0.15*ssim = 0.45*DocSim + 0.40*entropy + 0.15*ssim
  all: 0.35*DocSim + 0.30*entropy + 0.20*table_cell + 0.15*ssim (unchanged from D121)
  all_no_mask: 0.30*DocSim + 0.40*entropy + 0.20*table_cell + 0.10*ssim (unchanged from D121)
  formula/table: unchanged (β_ssim=0)

KEEP if spearman_mean > 0.4903 (D121 baseline). REVERT → text β-axis peak at 0.10.
...
METHOD_ID = "P1_126_dinov2_patch_text_beta05"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
BETA_TABLE_CELL = {
    "table": 0.0,
    "all": 0.15,         # see _fuse for weights
    "text": 0.0,
    "formula": 0.0,
    "all_no_mask": 0.10,
}[variant]
# D111: SSIM page-level supplement for all/all_no_mask
# all: (1-0.30-0.20-0.05)*DocSim + 0.30*entropy + 0.20*table_cell + 0.05*ssim
# all_no_mask: (1-0.40-0.20-0.05)*DocSim + 0.40*entropy + 0.20*table_cell + 0.05*ssim
BETA_SSIM = {
    "table": 0.0,
    "all": 0.15,
    "text": 0.15,       # D122 KEEP: text β=0.15
    "formula": 0.05,    # D123 KEEP: formula β=0.05 (D124 stacks both improvements)
    "all_no_mask": 0.10,
}[variant]
BETA_DINO_PATCH = {
    "table": 0.0,
...
def _dinov2_patch_mean_cosine_batch(orig_pils, recon_pils):
    """DINOv2 positional patch-level mean cosine similarity (D126)."""
    import torch.nn.functional as _F
    def _enc(pils):
        batch = torch.stack([_dino_transform(p.convert("RGB")) for p in pils]).to(device)
        with torch.no_grad():
            feat_dict = _dino_model.forward_features(batch)
        patches = feat_dict["x_norm_patchtokens"]  # [B, n_patches, dim]
        return _F.normalize(patches.float(), dim=-1)
    orig_p = _enc(orig_pils)
    recon_p = _enc(recon_pils)
    patch_sims = (orig_p * recon_p).sum(dim=-1)  # [B, n_patches]
    return patch_sims.mean(dim=-1).cpu().tolist()
...
def _fuse(prod_cc, page_name, diqa_sim=0.0, table_cell_score=0.0, ssim_score=0.0, dino_patch_score=0.0):
    ...
    For variant=all/all_no_mask with BETA_TABLE_CELL>0 and/or BETA_SSIM>0:
        clip_cosine = (1-alpha-beta_t-beta_g)*prod_cc + alpha*conf + beta_t*table_cell + beta_g*ssim
    Otherwise:
    ...
    if GAMMA_DIQA > 0.0 and BETA_SSIM > 0.0:
        # formula variant with both D106 IQ and SSIM supplements
    ...
    if BETA_TABLE_CELL > 0.0 or BETA_SSIM > 0.0 or BETA_DINO_PATCH > 0.0:
        beta_t = BETA_TABLE_CELL if table_cell_score > 0.0 else 0.0
        beta_g = BETA_SSIM  # SSIM supplement always applies (all pages)
        beta_dp = BETA_DINO_PATCH  # DINOv2 patch supplement (D126)
        if beta_t > 0.0 or beta_g > 0.0 or beta_dp > 0.0:
            return (1.0 - ALPHA_ENTROPY - beta_t - beta_g - beta_dp) * prod_cc + ALPHA_ENTROPY * conf + beta_t * table_cell_score + beta_g * ssim_score + beta_dp * dino_patch_score
...
results = []
orig_pils_buf, recon_pils_buf, meta_buf, name_buf, table_cell_score_buf, ssim_score_buf = [], [], [], [], [], []


def _flush_batch():
    global orig_pils_buf, recon_pils_buf, meta_buf, name_buf, table_cell_score_buf, ssim_score_buf
    if not orig_pils_buf:
        return
    if USE_DOCSIM_PAGE:
        cos_sims = _docsim_cosine_batch(orig_pils_buf, recon_pils_buf)
        if BETA_DINO_PATCH > 0.0:
            dino_patch_sims = _dinov2_patch_mean_cosine_batch(orig_pils_buf, recon_pils_buf)
        else:
            dino_patch_sims = [0.0] * len(orig_pils_buf)
        for meta, cs, name, tcs, ghs, dps in zip(meta_buf, cos_sims, name_buf, table_cell_score_buf, ssim_score_buf, dino_patch_sims):
            meta["clip_compare"] = {"clip_cosine": float(_fuse(float(cs), name, table_cell_score=tcs, ssim_score=ghs, dino_patch_score=dps))}
            results.append(meta)
    elif variant == "table":
        ...
        # variant=formula: 50/50 CLIP+DINOv2 fusion + D106 IQ term + optional SSIM
        clip_sims = _clip_cosine_batch(orig_pils_buf, recon_pils_buf)
