#!/usr/bin/env python3
"""D114 — SSIM page supplement beta=0.20 (D111d).

Computes SSIM (Structural Similarity Index) between grayscale 512px versions
of original and reconstructed pages. SSIM captures luminance, contrast, and
local structure similarity in a sliding window — complementary to DocSim's
semantic feature-based similarity and Sobel's gradient-edge structure.

Uses this as an additional β_ssim supplement for all/all_no_mask variants
on top of D107b (β_table=0.20). 100% page coverage (no element detection needed).
skimage.metrics.structural_similarity on raw grayscale (not binarized).

  all: (1-0.30-0.20-β_ssim)*DocSim + 0.30*entropy + 0.20*table_cell + β_ssim*ssim
  all_no_mask: (1-0.40-0.20-β_ssim)*DocSim + 0.40*entropy + 0.20*table_cell + β_ssim*ssim

β_ssim=0.05 (small supplement to preserve DocSim dominance).
KEEP if > 0.4820 (D107b best). REVERT → SSIM page supplement closed.
...
METHOD_ID = "P1_119_msssim_variant_specific_beta"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
def _ssim_page_supplement(orig_pil, recon_pil):
    """D119: MS-SSIM between RGB pages at SSIM_SIZE (multi-scale SSIM via piq.multi_scale_ssim)."""
    og = torch.from_numpy(
        np.array(orig_pil.convert("RGB").resize((SSIM_SIZE, SSIM_SIZE), Image.BILINEAR), dtype=np.float32) / 255.0
    ).permute(2, 0, 1).unsqueeze(0)
    rg = torch.from_numpy(
        np.array(recon_pil.convert("RGB").resize((SSIM_SIZE, SSIM_SIZE), Image.BILINEAR), dtype=np.float32) / 255.0
    ).permute(2, 0, 1).unsqueeze(0)
    return float(piq.multi_scale_ssim(og, rg, data_range=1.0))
...
def _fuse(prod_cc, page_name, diqa_sim=0.0, table_cell_score=0.0, ssim_score=0.0):
    ...
    if BETA_TABLE_CELL > 0.0 or BETA_SSIM > 0.0:
        ...
        if beta_t > 0.0 or beta_g > 0.0:
            return (1.0 - ALPHA_ENTROPY - beta_t - beta_g) * prod_cc + ALPHA_ENTROPY * conf + beta_t * table_cell_score + beta_g * ssim_score
...
def _flush_batch():
    ...
    if USE_DOCSIM_PAGE:
        ...
        for meta, cs, name, tcs, ghs in zip(meta_buf, cos_sims, name_buf, table_cell_score_buf, ssim_score_buf):
            meta["clip_compare"] = {"clip_cosine": float(_fuse(float(cs), name, table_cell_score=tcs, ssim_score=ghs))}
            results.append(meta)
    elif variant == "table":
