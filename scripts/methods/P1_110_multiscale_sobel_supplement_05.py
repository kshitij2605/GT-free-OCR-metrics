#!/usr/bin/env python3
"""D110 — Multi-scale Sobel edge-map cosine similarity supplement.

Computes Sobel edge maps at 3 spatial scales (1x, 0.5x, 0.25x) on grayscale
versions of original and reconstructed page images, then averages cosine
similarities across scales. Multi-scale captures both fine character-level edges
(1x) and coarse layout-block edges (0.25x), more robust than single-scale D109.

Uses this as an additional β_ms_edge supplement for all/all_no_mask variants
on top of D107b (β_table=0.20). 100% page coverage (no element detection needed).
scipy.ndimage.sobel for X and Y gradient, magnitude = hypot(Gx, Gy).

  all: (1-0.30-0.20-β_ms_edge)*DocSim + 0.30*entropy + 0.20*table_cell + β_ms_edge*ms_sobel
  all_no_mask: (1-0.40-0.20-β_ms_edge)*DocSim + 0.40*entropy + 0.20*table_cell + β_ms_edge*ms_sobel

β_ms_edge=0.05 (small supplement to preserve DocSim dominance).
KEEP if > 0.4820 (D107b best). REVERT → multi-scale Sobel supplement closed.
...
METHOD_ID = "P1_110_multiscale_sobel_supplement_05"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
DIQA_EPS = 1e-6
# D68: β_table=0.20 (D107b best) + β_grid=0.05 spatially-aware hash supplement
BETA_TABLE_CELL = {
    "table": 0.0,
    "all": 0.20,         # see _fuse for weights
    "text": 0.0,
    "formula": 0.0,
    "all_no_mask": 0.20,
}[variant]
# D110: multi-scale Sobel edge cosine supplement for all/all_no_mask
# all: (1-0.30-0.20-0.05)*DocSim + 0.30*entropy + 0.20*table_cell + 0.05*ms_sobel
# all_no_mask: (1-0.40-0.20-0.05)*DocSim + 0.40*entropy + 0.20*table_cell + 0.05*ms_sobel
BETA_SOBEL_EDGE = {
    "table": 0.0,
    "all": 0.05,
    "text": 0.0,
...
_MS_SOBEL_SCALES = (1.0, 0.5, 0.25)


def _sobel_cosine(arr_o, arr_r):
    """Cosine similarity between two Sobel edge maps (float32 arrays)."""
    eo = np.hypot(scipy_sobel(arr_o, axis=1), scipy_sobel(arr_o, axis=0))
    er = np.hypot(scipy_sobel(arr_r, axis=1), scipy_sobel(arr_r, axis=0))
    o, r = eo.ravel(), er.ravel()
    denom = np.linalg.norm(o) * np.linalg.norm(r)
    if denom < 1e-9:
        return 1.0
    return float(np.dot(o, r) / denom)


def _sobel_edge_similarity(orig_pil, recon_pil):
    """D110: average cosine similarity of Sobel edge maps at 3 spatial scales."""
    orig_g_pil = orig_pil.convert("L")
    recon_g_pil = recon_pil.convert("L")
    sims = []
    for scale in _MS_SOBEL_SCALES:
        if scale == 1.0:
            og = np.array(orig_g_pil, dtype=np.float32)
            rg = np.array(recon_g_pil, dtype=np.float32)
        else:
            W, H = orig_g_pil.size
            new_w, new_h = max(8, int(W * scale)), max(8, int(H * scale))
            og = np.array(orig_g_pil.resize((new_w, new_h), Image.BILINEAR), dtype=np.float32)
            rg = np.array(recon_g_pil.resize((new_w, new_h), Image.BILINEAR), dtype=np.float32)
        sims.append(_sobel_cosine(og, rg))
...
def _fuse(prod_cc, page_name, diqa_sim=0.0, table_cell_score=0.0, sobel_edge_score=0.0):
    ...
    For variant=all/all_no_mask with BETA_TABLE_CELL>0 and/or BETA_SOBEL_EDGE>0:
        clip_cosine = (1-alpha-beta_t-beta_g)*prod_cc + alpha*conf + beta_t*table_cell + beta_g*sobel_edge
    Otherwise:
    ...
    if BETA_TABLE_CELL > 0.0 or BETA_SOBEL_EDGE > 0.0:
        beta_t = BETA_TABLE_CELL if table_cell_score > 0.0 else 0.0
        beta_g = BETA_SOBEL_EDGE  # sobel edge always applies (all pages)
        if beta_t > 0.0 or beta_g > 0.0:
            return (1.0 - ALPHA_ENTROPY - beta_t - beta_g) * prod_cc + ALPHA_ENTROPY * conf + beta_t * table_cell_score + beta_g * sobel_edge_score
...
results = []
orig_pils_buf, recon_pils_buf, meta_buf, name_buf, table_cell_score_buf, sobel_edge_score_buf = [], [], [], [], [], []


def _flush_batch():
    global orig_pils_buf, recon_pils_buf, meta_buf, name_buf, table_cell_score_buf, sobel_edge_score_buf
    if not orig_pils_buf:
        return
    if USE_DOCSIM_PAGE:
        cos_sims = _docsim_cosine_batch(orig_pils_buf, recon_pils_buf)
        for meta, cs, name, tcs, ghs in zip(meta_buf, cos_sims, name_buf, table_cell_score_buf, sobel_edge_score_buf):
            meta["clip_compare"] = {"clip_cosine": float(_fuse(float(cs), name, table_cell_score=tcs, sobel_edge_score=ghs))}
            results.append(meta)
    elif variant == "table":
        ...
        # variant=formula: 50/50 CLIP+DINOv2 fusion + D106 IQ term
        clip_sims = _clip_cosine_batch(orig_pils_buf, recon_pils_buf)
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DOCSIM_BBOX:
        ...
        if score is not None:
            ...
            sobel_edge_score_buf.append(0.0)
        ...
        ghs = 0.0
        if BETA_SOBEL_EDGE > 0.0:
            ghs = _sobel_edge_similarity(orig_pil, recon_pil)
        sobel_edge_score_buf.append(ghs)
        orig_pils_buf.append(orig_pil)
        recon_pils_buf.append(recon_pil)
        meta_buf.append(meta)
        name_buf.append(page_dir.name)
