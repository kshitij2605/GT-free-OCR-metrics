#!/usr/bin/env python3
"""D123 — SSIM supplement extended to formula variant at β=0.05.

Base: D121 stack (text=0.10, all=0.15, all_no_mask=0.10, mean 0.4903 NEW BEST).
D123 tests whether formula also benefits from SSIM structural supplement.

  formula: 0.15*DocSim + 0.30*entropy + 0.50*D106_iq + 0.05*ssim
           (reduces prod_cc weight 0.20→0.15; GAMMA_DIQA=0.50 unchanged)
  text: 0.50*DocSim + 0.40*entropy + 0.10*ssim (D121 unchanged)
  all: 0.35*DocSim + 0.30*entropy + 0.20*table_cell + 0.15*ssim (unchanged)
  all_no_mask: 0.30*DocSim + 0.40*entropy + 0.20*table_cell + 0.10*ssim (unchanged)
  table: unchanged (β_ssim=0)

Parallel test alongside D122 (text β=0.15) — independent mechanism.
KEEP if spearman_mean > 0.4903 (D121 baseline).
"""
...
METHOD_ID = "P1_123_ssim_formula_beta05"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
BETA_SSIM = {
    "table": 0.0,
    "all": 0.15,
    "text": 0.10,       # D121 base: text β=0.10 (D123 tests formula, not text)
    "formula": 0.05,    # D123 NEW: formula SSIM supplement at β=0.05
    "all_no_mask": 0.10,
}[variant]
