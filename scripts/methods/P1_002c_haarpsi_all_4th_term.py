#!/usr/bin/env python3
"""P1_002c_haarpsi_all_4th_term: H13.2 — HaarPSI as 4th term in mc REPLACING DISTS for variant=all only.

H13.2 hypothesis: H5.b confirmed DISTS-as-4th-term lifts variant=all mc by +0.065 (the
ONLY confirmed mc 4th-term win across all 5 variants). HaarPSI (D2, priority #2,
Reisenhofer et al. 2018, Signal Processing) uses 6 discrete 2D Haar wavelet filters and
weights local similarity by low-frequency coefficients — fundamentally orthogonal to
DISTS's untrained-VGG-statistics signal class. Test whether HaarPSI as a drop-in
replacement for DISTS in the variant=all 4th-term slot beats DISTS-as-4th-term for that
variant.

H13.1 showed HaarPSI as 4th-term for variant=all_no_mask DROPPED mc 0.264->0.259, but
that variant is cc-bound (cc=0.308 wins). variant=all is mc-bound (mc=0.283, cc=0.203),
so a 4th-term mc lift directly raises spearman_mean.

Diff vs P1_120c3b (H11.2 baseline = current best 0.3600):
- For variant=all only: REPLACE DISTS with HaarPSI in the 4th-term slot.
  was: 0.3*SSIM + 0.2*(1-MSE) + 0.2*(1-LPIPS) + 0.3*(1-DISTS)   (H5.b)
  now: 0.3*SSIM + 0.2*(1-MSE) + 0.2*(1-LPIPS) + 0.3*HaarPSI     (H13.2)
- All other variants UNCHANGED from H11.2:
  variant=text/formula/table: 0.4*SSIM + 0.3*(1-MSE) + 0.3*(1-LPIPS) (original 3-term)
  variant=all_no_mask: same 3-term, BUT cc path uses H6.1+H11.2 preprocessing
...
Predicted: variant=all mc 0.283 (H5.b DISTS) → 0.27-0.30 (HaarPSI may match or beat DISTS).
spearman_mean 0.3600 → 0.357-0.366.
"""
...
METHOD_ID = "P1_002c_haarpsi_all_4th_term"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
USE_HAARPSI = variant == "all"        # H13.2: HaarPSI 4th term ONLY for 'all' (replaces DISTS)
...
log.info(
    "device=%s variant=%s method=%s use_haarpsi=%s use_clip_only=%s use_preproc=%s",
    device, variant, METHOD_ID, USE_HAARPSI, USE_CLIP_ONLY, USE_PREPROC,
)

_preprocessor = ImagePreprocessor()


# ── HaarPSI helper (only used for variant=all) ────────────────────────────────
_haarpsi_transform = T.Compose([
    T.Resize((SSIM_SIZE, SSIM_SIZE), interpolation=T.InterpolationMode.BICUBIC),
    T.ToTensor(),  # [0,1]
])


def _haarpsi_score(orig_pil: Image.Image, recon_pil: Image.Image) -> float:
    """Compute piq.haarpsi (similarity, higher=better, in [0,1])."""
    ...
    with torch.no_grad():
        sim = piq.haarpsi(orig_t, recon_t, data_range=1.0, reduction="none")
    return float(np.clip(sim.item(), 0.0, 1.0))
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_HAARPSI:
        # variant=all: HaarPSI replaces DISTS as 4th term (H13.2)
        haarpsi_val = _haarpsi_score(orig_pil, recon_pil)
        composite = (
            0.3 * ssim_val
            + 0.2 * max(0.0, 1.0 - mse_val)
            + 0.2 * max(0.0, 1.0 - lpips_val)
            + 0.3 * haarpsi_val  # similarity, direct add (no 1-x flip)
        )
        multi_metric = {
            "ssim": ssim_val,
            "mse": mse_val,
