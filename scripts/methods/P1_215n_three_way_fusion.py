...
BATCH_SIZE = 16
DINO_SIZE = 224
# Three-way fusion of production cc + window-MIN(mean-conf) + window-MIN(Shannon).
...
# Per-variant α budget (matches D215.k; table=0 disables entropy) split 50/50
# between mean-confidence and Shannon-top-logprobs sources.
ALPHA_MEAN = {
    "table": 0.0,
    "all": 0.15,
    "text": 0.20,
...
log.info(
    "device=%s variant=%s method=%s alpha_mean=%.2f alpha_shannon=%.2f window_k=%d use_dists=%s use_preproc=%s use_docsim_page=%s use_docsim_bbox=%s",
    device, variant, METHOD_ID, ALPHA_MEAN, ALPHA_SHANNON, WINDOW_K,
...
def _window_min_two_sources(page_name):
    """Returns (mean_conf_min, shannon_conf_min) — two parallel windowed-MIN
    aggregations over the SAME per-token token-list, but with two different
    per-token confidence proxies:
      mean_conf:    exp(top_logprob[0])  — the chosen-token probability
      shannon_conf: 1 - H/log(K_top)     — distribution-shape peakedness

    Aggregation: for each source, compute the rolling K-token mean and return
    the MIN over windows. Returns (0.5, 0.5) if logprobs missing.
    """
    ...
    for t in data.get("tokens", []):
        ...
        if s <= 0:
            continue
        means.append(float(probs[0]))  # exp(top_logprob[0]) — chosen-token prob
        norm = [p / s for p in probs]
        H = -sum(p * math.log(p + 1e-12) for p in norm if p > 0)
        log_k = math.log(len(top))
        if log_k <= 0:
            continue
        shannons.append(1.0 - H / log_k)

    def _windowed_min(arr_list):
        if not arr_list:
            return 0.5
        if len(arr_list) < WINDOW_K:
    ...
    return _windowed_min(means), _windowed_min(shannons)


def _fuse(prod_cc, page_name):
    """Three-way convex combo of production cc, window-MIN(mean-conf), and
    window-MIN(Shannon-top-logprobs).
        fused = (1 - α_mean - α_shannon) * prod_cc
                + α_mean * mean_conf_min
                + α_shannon * shannon_conf_min
    """
    mean_conf, shannon_conf = _window_min_two_sources(page_name)
    return (
        (1.0 - ALPHA_MEAN - ALPHA_SHANNON) * prod_cc
        + ALPHA_MEAN * mean_conf
        + ALPHA_SHANNON * shannon_conf
    )
...
log.info("Done. %d pages -> %s (alpha_mean=%.2f, alpha_shannon=%.2f, window_k=%d)",
         len(results), out_path, ALPHA_MEAN, ALPHA_SHANNON, WINDOW_K)
