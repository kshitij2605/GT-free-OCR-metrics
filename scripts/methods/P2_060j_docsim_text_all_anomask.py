#!/usr/bin/env python3
"""P2_060j — D60.j extends D60.b's DocSim-cc set to variant=text.

Identical to P2_060b EXCEPT: USE_DOCSIM also fires for variant=text. Other
variants unchanged (formula keeps H4.e fusion, table keeps H6.1 CLIP-only).

Empirical justification (post-fix dataset audit on D60 uniform):
    DocSim cc text       = 0.4139   (lift over baseline cc=0.1579 AND
                                     over baseline mc=0.3886)
    DocSim cc all        = 0.3934   (= D60.b confirmed)
    DocSim cc all_no_mask= 0.3937   (= D60.b confirmed)
    DocSim cc formula    = 0.2986   (HURTS — keep baseline cc=0.4388)
    DocSim cc table      = 0.1450   (HURTS — keep baseline cc=0.2678)

The pre-fix dataset compressed the variant=text DocSim signal to ~tie
with baseline (D60 uniform's old text=0.3887 vs baseline 0.3889) so the
synthesizer parked variant=text as low-priority. Post-fix audit revealed
the latent text DocSim cc value at 0.4139 — a clear winner.

Predicted spearman_mean: 0.4093 (+0.0051 vs D60.b post-fix anchor 0.4042).
"""
...
METHOD_ID = "P2_060j_docsim_text_all_anomask"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
