...
METHOD_ID = "P1_148_elem_p10_table_beta05"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
BETA_CONTENT_ELEM = {
    "table": 0.05,  # D148: table elem_p10 β=0.05
    "text": 0.0,
    "formula": 0.0,
    "all": 0.10,
    "all_no_mask": 0.10,
...
def _fuse(prod_cc, page_name, diqa_sim=0.0, table_cell_score=0.0, ssim_score=0.0, content_elem_score=0.0):
    ...
    if GAMMA_DIQA > 0.0 and BETA_SSIM > 0.0:
        # D148: include BETA_CONTENT_ELEM in the DIQA+SSIM branch (table variant)
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DOCSIM_BBOX:
        ...
        if score is not None:
            ...
            ces_fb = _content_element_clip_p10(orig_pil, recon_pil, page_dir) if BETA_CONTENT_ELEM > 0.0 else 0.0
            content_elem_score_buf.append(ces_fb)
