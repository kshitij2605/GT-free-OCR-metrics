...
METHOD_ID = "P2_213_elem_p05_table_fixed"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
BETA_SSIM = {
    "table": 0.15,     # D133: table SSIM β sweep 0.10→0.15 (D132 KEEP still climbing)
    "all": 0.20,       # D136 KEEP: all β=0.20 marginally improved all 0.4458→0.4461
    "text": 0.15,       # D122 KEEP: text β=0.15
    "formula": 0.05,    # D123 KEEP: formula β=0.05 (D124 stacks both improvements)
    "all_no_mask": 0.10,
}[variant]
# D137: per-element CLIP P10 supplement for all/all_no_mask
# CLIP cosine P10 across all content bboxes (text+formula+table elements combined).
# Mirrors per-cell-MIN breakthrough from D60 but for mixed-content variants.
# all: (1-0.3-0.15-0.20-0.10)*DocSim + 0.3*entropy + 0.15*table_cell + 0.20*ssim + 0.10*elem_p10
#    = 0.25*DocSim + 0.30*entropy + 0.15*table_cell + 0.20*ssim + 0.10*elem_p10
# all_no_mask: 0.30*DocSim + 0.40*entropy + 0.10*table_cell + 0.10*ssim + 0.10*elem_p10
BETA_CONTENT_ELEM = {
    "table": 0.05,  # D209: add elem_clip_p5 to table (β=0.05, same as D148 delta)
    "text": 0.0,
    "formula": 0.0,
    "all": 0.10,
    "all_no_mask": 0.10,
...
def _content_element_clip_p10(orig_pil, recon_pil, page_dir):
    ...
    Extends the per-cell-MIN idea from D60.p to all content types for
    variant=all/all_no_mask. Uses P10 (10th percentile) instead of MIN to
    be robust to heterogeneous element types with different natural CLIP ranges.
    Returns 0.0 (neutral — no supplemental weight) if no valid crops found.
    """
    bboxes = []
...
def _flush_batch():
    ...
    if USE_DOCSIM_PAGE:
        ...
        for meta, clip_cos, name, orig_p, recon_p, ghs, ces in zip(
            meta_buf, clip_sims, name_buf, orig_pils_buf, recon_pils_buf, ssim_score_buf, content_elem_score_buf
        ):
            diqa_sim_t = _diqa_similarity(orig_p, recon_p) if GAMMA_DIQA > 0.0 else 0.0
            meta["clip_compare"] = {"clip_cosine": float(_fuse(float(clip_cos), name, diqa_sim_t, ssim_score=ghs, content_elem_score=ces))}
            results.append(meta)
    else:
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DOCSIM_BBOX:
        ...
        if score is not None:
            ...
            ces_t = _content_element_clip_p10(orig_pil, recon_pil, page_dir) if BETA_CONTENT_ELEM > 0.0 else 0.0
            meta["clip_compare"] = {"clip_cosine": float(_fuse(float(score), page_dir.name, diqa_sim_t, ssim_score=ssim_t, content_elem_score=ces_t))}
            results.append(meta)
        else:
            n_fallback_pages += 1
            table_cell_score_buf.append(0.0)
            ssim_score_buf.append(ssim_val if BETA_SSIM > 0.0 else 0.0)
            content_elem_score_buf.append(_content_element_clip_p10(orig_pil, recon_pil, page_dir) if BETA_CONTENT_ELEM > 0.0 else 0.0)
