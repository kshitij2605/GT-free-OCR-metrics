#!/usr/bin/env python3
"""P1_084f_dinov2_only_all: H8.1 — DINOv2 CLS-only for variant=all.

H8.1 hypothesis: Replacing the 50/50 CLIP+DINOv2 fusion with DINOv2 CLS-only
for variant=all will lift variant=all clip_cosine Spearman from 0.203 toward
DINOv2's per-element formula strength (0.543), because DINOv2 CLS captures
semantic document structure better than a 50/50 blend for full-page end-to-end
comparison.
...
- variant=all: DINOv2 CLS-only cosine (H8.1: NO CLIP blend for this variant)
- all other variants (text, formula, all_no_mask): 0.5 * DINOv2_vitb14_CLS + 0.5 * OpenCLIP_ViT-B/32 (raw RGB)
"""
...
METHOD_ID = "P1_084f_dinov2_only_all"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
USE_DINOV2_ONLY = variant == "all"   # H8.1: DINOv2 CLS-only for variant=all
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_clip_only=%s use_table_preproc=%s use_dinov2_only=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_CLIP_ONLY, USE_TABLE_PREPROC, USE_DINOV2_ONLY,
)
...
def _flush_batch():
    ...
    if USE_CLIP_ONLY:
        ...
        for meta, clip_cos in zip(meta_buf, clip_sims):
            ...
            results.append(meta)
    elif USE_DINOV2_ONLY:
        # variant=all: H8.1 — DINOv2 CLS only (no CLIP blend)
        ...
        # text/formula/all_no_mask: 50/50 CLIP + DINOv2 average (H4.e strategy, raw RGB)
        clip_sims = _clip_cosine_batch(orig_pils_buf, recon_pils_buf)
        dino_sims = _dinov2_cosine_batch(orig_pils_buf, recon_pils_buf)
