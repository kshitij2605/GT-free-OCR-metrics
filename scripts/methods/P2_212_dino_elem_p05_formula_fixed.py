...
METHOD_ID = "P2_212_dino_elem_p05_formula_fixed"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
BETA_CONTENT_ELEM = {
    "table": 0.0,   # D211: revert to D147 base (no table elem_clip)
    "text": 0.0,
    "formula": 0.05,  # D211: add DINOv2 elem_p5 to formula (β=0.05)
    "all": 0.10,
    "all_no_mask": 0.10,
}[variant]
...
def _content_element_dino_p5(orig_pil, recon_pil, page_dir):
    """DINOv2 CLS cosine P5 over formula-element bboxes.

    DINOv2 is formula-specialist (CLS Spearman 0.5407 on formula vs CLIP 0.4235).
    Uses only formula bboxes to target glyph-level rendering quality.
    Returns 0.0 if no formula elements found.
    """
    bboxes = []
    ep = page_dir / "ocr_formula_elements.json"
    if ep.exists():
        try:
            with ep.open("r") as f:
                elems = json.load(f)
    ...
    sims = []
    for s in range(0, len(orig_crops), BATCH_SIZE):
        sims.extend(_dinov2_cosine_batch(orig_crops[s:s + BATCH_SIZE], recon_crops[s:s + BATCH_SIZE]))
    return float(np.percentile(sims, 5))
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DOCSIM_BBOX:
        ...
        if BETA_CONTENT_ELEM > 0.0:
            if variant == "formula":
                ces = _content_element_dino_p5(orig_pil, recon_pil, page_dir)
            else:
