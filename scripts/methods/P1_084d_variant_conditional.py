#!/usr/bin/env python3
"""P1_084d: Variant-conditional encoder in clip_cosine slot.

H4.d hypothesis: selecting the best encoder per variant recovers CLIP's table
strength while keeping DINOv2's formula gain.

  USE_DINOV2 = variant in {"formula", "all_no_mask"}
  USE_CLIP   = variant in {"text", "table", "all"}

clip_cosine = DINOv2_vitb14_CLS_cosine  if USE_DINOV2 else OpenCLIP_ViT-B/32_cosine

multi_composite (SSIM/MSE/LPIPS) is UNCHANGED vs baseline / H4.c.
Output JSON shape unchanged: clip_compare={"clip_cosine": <chosen encoder cosine>}.
"""
...
METHOD_ID = "P1_084d_variant_conditional"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
# ── Variant-conditional encoder selection ─────────────────────────────────────
USE_DINOV2 = variant in {"formula", "all_no_mask"}
...
log.info("device=%s variant=%s method=%s use_dinov2=%s", device, variant, METHOD_ID, USE_DINOV2)

_preprocessor = ImagePreprocessor()

# ── Load only the chosen encoder ──────────────────────────────────────────────
if USE_DINOV2:
    log.info("Loading DINOv2 vitb14 (variant=%s)...", variant)
    ...
    import open_clip
    log.info("Loading OpenCLIP ViT-B-32 (variant=%s)...", variant)
...
# ── Encoder cosine (batched) ──────────────────────────────────────────────────

def _cosine_batch(orig_pils: list, recon_pils: list) -> list:
    if USE_DINOV2:
        def _enc(pils):
...
def _flush_batch():
    ...
    if not orig_pils_buf:
        return
    cosines = _cosine_batch(orig_pils_buf, recon_pils_buf)
    for meta, cos_val in zip(meta_buf, cosines):
        meta["clip_compare"] = {"clip_cosine": float(cos_val)}
        results.append(meta)
    orig_pils_buf, recon_pils_buf, meta_buf = [], [], []
