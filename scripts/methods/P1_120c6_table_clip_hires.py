#!/usr/bin/env python3
"""P1_120c6_table_clip_hires: H6.4 — ViT-B/32 @ 336x336 input for variant=table.

H6.4 hypothesis: Increasing input resolution from 224->336 for the OpenCLIP ViT-B/32
encoder on variant=table will improve table cc beyond H6.1's 0.277. At 336x336, ViT-B/32
sees ~(336/32)^2 = 110 patches vs ~(224/32)^2 = 49 patches — ~2.25x more spatial detail
flowing into the same global CLS pool. The encoder family is unchanged (ViT-B/32 proved
superior to ViT-L/14 in H6.2), only resolution increases.
...
clip_cosine logic:
- variant=table: CLIP cosine ONLY, with baseline preprocessing applied before _clip_preprocess_hi
                 (grayscale -> SHARPEN -> autocontrast -> RGB), using ViT-B/32 @ 336x336
- all other variants: 0.5 * DINOv2_vitb14_CLS + 0.5 * OpenCLIP_ViT-B/32 @ 224 (raw RGB, unchanged)
"""
...
METHOD_ID = "P1_120c6_table_clip_hires"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_clip_only=%s use_table_preproc=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_CLIP_ONLY, USE_TABLE_PREPROC,
)
...
# ── OpenCLIP setup ─────────────────────────────────────────────────────────────
# Standard 224 model for non-table variants
log.info("Loading OpenCLIP ViT-B-32 (laion2b_s34b_b79k) @ 224...")
_clip_model, _, _clip_preprocess = open_clip.create_model_and_transforms(
    "ViT-B-32", pretrained="laion2b_s34b_b79k"
)
_clip_model = _clip_model.eval().to(device)

# H6.4: hi-res 336x336 model for table variant only
if USE_TABLE_PREPROC:
    log.info("Loading OpenCLIP ViT-B-32 (laion2b_s34b_b79k) @ 336 for table variant (H6.4)...")
    _clip_model_hi, _, _clip_preprocess_hi = open_clip.create_model_and_transforms(
        "ViT-B-32", pretrained="laion2b_s34b_b79k", force_image_size=336
    )
    _clip_model_hi = _clip_model_hi.eval().to(device)
else:
    _clip_model_hi = None
    _clip_preprocess_hi = None
...
def _clip_cosine_batch(orig_pils: list, recon_pils: list) -> list:
    def _enc(pils):
        if USE_TABLE_PREPROC:
            # H6.4: apply baseline grayscale+SHARPEN+autocontrast->RGB before hi-res CLIP preprocess
            processed = [_baseline_clip_preprocess(p) for p in pils]
            batch = torch.stack([_clip_preprocess_hi(p) for p in processed]).to(device)
            with torch.no_grad():
                feats = _clip_model_hi.encode_image(batch)
        else:
...
def _flush_batch():
    ...
    if USE_CLIP_ONLY:
        # table variant: use CLIP cosine alone (H4.e strategy), with H6.4 hi-res preprocessing
