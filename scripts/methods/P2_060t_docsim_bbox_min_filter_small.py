#!/usr/bin/env python3
"""P2_060t — D60.t: D60.p + filter tiny bboxes before MIN aggregation.

Refinement of D60.p (best 0.4453, table cc 0.4767). Drops bboxes whose
shorter side is < 32 px before MIN aggregation. Hypothesis: very small
bboxes (artifact bbox detections, ruled lines, single characters) are
visually noisy at the DocSim head's input scale and may drag the MIN
score below the actual cell-quality signal. Filtering eliminates noise
sources without changing the dominant aggregation logic.

If after filtering a page has zero usable bboxes, fall back to the D60.p
baseline path (H6.1 CLIP-only) — same fallback rule as D60.p for empty
bbox pages. Other 4 variants byte-equal to D60.p.

Predicted spearman_mean: 0.443-0.460, most likely 0.448 (+0.003). KEEP if
>= 0.450. Kill switch: table cc <= 0.45 (filter dropped real signal).
"""
...
METHOD_ID = "P2_060t_docsim_bbox_min_filter_small"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
BBOX_MIN_SIDE = 32  # D60.t: drop bboxes whose shorter side is below this many px
...
def _crop_bbox(pil: Image.Image, bbox) -> Image.Image | None:
    """Crop a bbox; return None if either side < BBOX_MIN_SIDE px."""
    ...
    y1 = max(0, min(y1, H))
    y2 = max(0, min(y2, H))
    if x2 - x1 < BBOX_MIN_SIDE or y2 - y1 < BBOX_MIN_SIDE:
        return None
