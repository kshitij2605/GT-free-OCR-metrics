...
METHOD_ID = "P2_060r_docsim_bbox_min_h61_crop"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
def _docsim_per_bbox_score(orig_pil: Image.Image, recon_pil: Image.Image, bboxes) -> float | None:
    """MIN DocSim cosine over per-table-bbox crops, with H6.1 preprocessing applied to crops.
    None if no usable bboxes."""
    orig_crops = []
    recon_crops = []
    for bb in bboxes:
        ...
        if oc is None or rc is None:
            continue
        # D60.r: apply H6.1 preprocessing (grayscale -> SHARPEN -> autocontrast -> RGB) to bbox crops
        # before passing to DocSim head.
        orig_crops.append(_baseline_clip_preprocess(oc))
        recon_crops.append(_baseline_clip_preprocess(rc))
    if not orig_crops:
        return None
