...
METHOD_ID = "P2_214_dinov2_patch_cosine"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
def _dinov2_patch_cosine_batch(orig_pils, recon_pils):
    """Soft-NN DINOv2 patch cosine: shift-tolerant page similarity via patch tokens."""
    def _enc(pils):
        batch = torch.stack([_dino_transform(p.convert("RGB")) for p in pils]).to(device)
        with torch.no_grad():
    ...
    return sims.max(dim=2).values.mean(dim=1).cpu().tolist()  # [B]
