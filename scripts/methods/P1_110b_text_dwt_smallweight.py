#!/usr/bin/env python3
"""P1_110b_text_dwt_smallweight: H7.1b — DWT 4th term with SSIM-preserving weights for variant=text.

H7.1b hypothesis: H7.1 (equal 4-way 0.25/0.25/0.25/0.25) regressed text Spearman 0.389->0.375
because halving SSIM weight (0.40->0.25) destroyed the dominant text signal. This experiment
preserves SSIM at 0.40 and adds DWT at only 0.10 (taken from MSE+LPIPS, 0.30->0.25 each):

  variant=text (USE_TEXT_DWT=True):
    composite = 0.40*SSIM + 0.25*(1-MSE) + 0.25*(1-LPIPS) + 0.10*dwt_detail_sim

  variant=all (USE_DISTS=True):
    composite = 0.3*SSIM + 0.2*(1-MSE) + 0.2*(1-LPIPS) + 0.3*(1-DISTS)   [H5.b, unchanged]

  all other variants:
    composite = 0.4*SSIM + 0.3*(1-MSE) + 0.3*(1-LPIPS)                    [original, unchanged]

clip_cosine logic:
- variant=table: CLIP cosine ONLY with baseline preprocessing (H6.1, unchanged)
- all other variants: 0.5*DINOv2_vitb14_CLS + 0.5*OpenCLIP_ViT-B/32 (raw RGB, unchanged)

DWT helper: db2 wavelet, levels 2 and 3, detail subbands cosine similarity, mapped to [0,1].
"""
...
METHOD_ID = "P1_110b_text_dwt_smallweight"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
USE_TEXT_DWT = variant == "text"        # H7.1b: DWT 4th term for text only (asymmetric weights)
...
log.info(
    "device=%s variant=%s method=%s use_text_dwt=%s use_dists=%s use_clip_only=%s use_table_preproc=%s",
    device, variant, METHOD_ID, USE_TEXT_DWT, USE_DISTS, USE_CLIP_ONLY, USE_TABLE_PREPROC,
)
...
# ── H7.1b: DWT detail similarity (db2 wavelet, levels 2+3, cosine on detail subbands) ──────

def _dwt_detail_sim(orig_pil: Image.Image, recon_pil: Image.Image) -> float:
    """Compute DWT detail subband cosine similarity.

    Uses db2 wavelet at decomposition levels 2 and 3.
    Concatenates all detail subbands (LH, HL, HH) at each level into a flat vector,
    computes cosine similarity, and maps from [-1,1] to [0,1].
    Returns 0.5 (neutral) if either image has zero-norm detail vector.
    """
    def _to_arr(p):
        g = p.convert("L").resize((512, 512), Image.BILINEAR)
        return np.array(g, dtype=np.float32) / 255.0

    o = _to_arr(orig_pil)
    r = _to_arr(recon_pil)
    cosines = []
    for level in (2, 3):
        co = pywt.wavedec2(o, wavelet="db2", level=level)
        cr = pywt.wavedec2(r, wavelet="db2", level=level)
        # co[0] is approximation; co[1:] are detail tuples at each level
        do = np.concatenate([np.concatenate([d.flatten() for d in t]) for t in co[1:]])
        dr = np.concatenate([np.concatenate([d.flatten() for d in t]) for t in cr[1:]])
        no = np.linalg.norm(do)
        nr = np.linalg.norm(dr)
        cosines.append(0.0 if no < 1e-8 or nr < 1e-8 else float((do @ dr) / (no * nr)))
    return float(np.clip(0.5 * (np.mean(cosines) + 1.0), 0.0, 1.0))
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_TEXT_DWT:
        # H7.1b: asymmetric weights — SSIM preserved at 0.40, DWT added at 0.10
        dwt_val = _dwt_detail_sim(orig_pil, recon_pil)
        composite = (
            0.40 * ssim_val
            + 0.25 * max(0.0, 1.0 - mse_val)
            + 0.25 * max(0.0, 1.0 - lpips_val)
            + 0.10 * dwt_val
        )
        multi_metric = {
            "ssim": ssim_val,
        ...
        # variant=all: DISTS-augmented rebalanced composite (H5.b, unchanged)
        ...
        # all other variants: original formula (0.4/0.3/0.3), unchanged from H6.1
