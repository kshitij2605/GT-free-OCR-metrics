#!/usr/bin/env python3
"""P1_084g_dinov2_only_all_no_mask: H11.1 — DINOv2 CLS-only encoder for variant=all_no_mask cc.

H11.1 hypothesis: Prior P1_084 leaderboard data shows DINOv2-CLS-only gives all_no_mask
cc=0.310, slightly above the 50/50 CLIP+DINOv2 fusion used by H4.e/H6.1 (all_no_mask
cc=0.3073). Using DINOv2-CLS-only (plain RGB, no CLIP, no fusion) for variant=all_no_mask
clip_cosine slot ONLY should break the 0.3073 ceiling without disturbing the other 4
variants' carefully-tuned settings.

Per-variant logic (all other variants BIT-FOR-BIT identical to P1_120c3):
- variant=text        -> 50/50 CLIP+DINOv2 fusion (H4.e default, unchanged)
- variant=formula     -> 50/50 CLIP+DINOv2 fusion (H4.e default, unchanged)
- variant=table       -> CLIP-only with H6.1 grayscale+SHARPEN+autocontrast->RGB preprocessing
- variant=all         -> 50/50 CLIP+DINOv2 fusion (H4.e default, unchanged)
- variant=all_no_mask -> DINOv2 vitb14 CLS-only, plain RGB (NEW)
...
- variant=table:     CLIP cosine ONLY, with baseline preprocessing applied before _clip_preprocess
                     (grayscale -> SHARPEN -> autocontrast -> RGB)  [H6.1, unchanged]
- variant=all_no_mask: DINOv2 vitb14 CLS cosine ONLY, plain RGB (no CLIP)  [H11.1 NEW]
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_clip_only=%s use_table_preproc=%s use_dino_only=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_CLIP_ONLY, USE_TABLE_PREPROC, USE_DINO_ONLY,
)
...
def _flush_batch():
    ...
    if USE_CLIP_ONLY:
        ...
        # H11.1: all_no_mask variant -> DINOv2 CLS-only, plain RGB (no CLIP, no fusion)
        dino_sims = _dinov2_cosine_batch(orig_pils_buf, recon_pils_buf)
        for meta, dino_cos in zip(meta_buf, dino_sims):
            meta["clip_compare"] = {"clip_cosine": float(dino_cos)}
            results.append(meta)
    else:
