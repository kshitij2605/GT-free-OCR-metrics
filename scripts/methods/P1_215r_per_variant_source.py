...
METHOD_ID = "P1_215r_per_variant_source"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
# D215.r per-variant SOURCE selection. text/formula/all_no_mask use Shannon
# (D215.l winners); all uses mean-confidence (D215.k winner); table is α=0.
SOURCE = {
    "table": "none",                       # disabled at α=0
    "all": "mean_confidence",              # D215.k 0.4221 > D215.l 0.4199
    "text": "shannon_top_logprobs",        # D215.l 0.4489 > D215.k 0.4434
    "formula": "shannon_top_logprobs",     # D215.l 0.5872 > D215.k 0.5685
    "all_no_mask": "shannon_top_logprobs", # D215.l 0.4382 > D215.k 0.4366
}[variant]
...
def _window_min_confidence(page_name):
    """D215.r per-variant SOURCE selection: MIN over rolling K-token window
    means of per-token confidence proxy. Source chosen by SOURCE constant:

      mean_confidence:        exp(top_logprob[0])  — chosen-token probability
      shannon_top_logprobs:   1 - H/log(K_top)     — distribution-shape peakedness

    Returns 0.5 (neutral) if logprobs missing or SOURCE == "none".
    """
    if SOURCE == "none":
        return 0.5
    ...
    for t in data.get("tokens", []):
        ...
        if s <= 0:
            continue
        if SOURCE == "mean_confidence":
            confs.append(float(probs[0]))
        elif SOURCE == "shannon_top_logprobs":
