...
METHOD_ID = "P1_108_formula_bbox_supplement_10"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
DIQA_EPS = 1e-6
# D108: β_table=0.20 (D107b best) + β_formula=0.10 formula-bbox supplement
BETA_TABLE_CELL = {
    "table": 0.0,
...
# D108: formula-bbox MIN DocSim supplement for all/all_no_mask
BETA_FORMULA_BBOX = {
    "table": 0.0,
    "all": 0.10,         # 0.40*page_DocSim + 0.30*entropy + 0.20*table_cell + 0.10*formula_bbox
    "text": 0.0,
    "formula": 0.0,
    "all_no_mask": 0.10, # 0.30*page_DocSim + 0.40*entropy + 0.20*table_cell + 0.10*formula_bbox
}[variant]
...
def _fuse(prod_cc, page_name, diqa_sim=0.0, table_cell_score=0.0, formula_bbox_score=0.0):
    ...
    if BETA_TABLE_CELL > 0.0 or BETA_FORMULA_BBOX > 0.0:
        beta_t = BETA_TABLE_CELL if table_cell_score > 0.0 else 0.0
        beta_f = BETA_FORMULA_BBOX if formula_bbox_score > 0.0 else 0.0
        if beta_t > 0.0 or beta_f > 0.0:
            return (1.0 - ALPHA_ENTROPY - beta_t - beta_f) * prod_cc + ALPHA_ENTROPY * conf + beta_t * table_cell_score + beta_f * formula_bbox_score
...
results = []
orig_pils_buf, recon_pils_buf, meta_buf, name_buf, table_cell_score_buf, formula_bbox_score_buf = [], [], [], [], [], []


def _flush_batch():
    global orig_pils_buf, recon_pils_buf, meta_buf, name_buf, table_cell_score_buf, formula_bbox_score_buf
    if not orig_pils_buf:
        return
    if USE_DOCSIM_PAGE:
        cos_sims = _docsim_cosine_batch(orig_pils_buf, recon_pils_buf)
        for meta, cs, name, tcs, fbs in zip(meta_buf, cos_sims, name_buf, table_cell_score_buf, formula_bbox_score_buf):
            meta["clip_compare"] = {"clip_cosine": float(_fuse(float(cs), name, table_cell_score=tcs, formula_bbox_score=fbs))}
            results.append(meta)
    elif variant == "table":
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DOCSIM_BBOX:
        ...
        if score is not None:
            ...
            formula_bbox_score_buf.append(0.0)
        ...
        fbs = 0.0
        if BETA_FORMULA_BBOX > 0.0:
            formula_bbox_path = page_dir / "ocr_formula_elements.json"
            formula_bboxes = []
            if formula_bbox_path.exists():
                try:
                    with formula_bbox_path.open("r") as _ff:
                        _felems = json.load(_ff)
                    formula_bboxes = [e.get("bbox") for e in _felems if e.get("bbox")]
                except Exception:
                    formula_bboxes = []
            if formula_bboxes:
                fbs = _docsim_per_bbox_score(orig_pil, recon_pil, formula_bboxes) or 0.0
        formula_bbox_score_buf.append(fbs)
        orig_pils_buf.append(orig_pil)
        recon_pils_buf.append(recon_pil)
        meta_buf.append(meta)
        name_buf.append(page_dir.name)
