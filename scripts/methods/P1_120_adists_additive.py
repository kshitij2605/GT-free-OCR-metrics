#!/usr/bin/env python3
"""P1_120_adists_additive: DISTS as additive 4th term in multi_composite.

H5 hypothesis: A-DISTS unavailable in pyiqa 0.1.15 (only 'dists' available).
Falls back to pyiqa DISTS added as 4th term (NOT slot replacement).

New composite formula (rebalanced, additive — keeps all original terms):
  composite = 0.3*SSIM + 0.2*(1-MSE) + 0.2*(1-LPIPS) + 0.3*(1-DISTS)

clip_cosine logic from H4.e (UNCHANGED):
...
METHOD_ID = "P1_120_adists_additive"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
# Per-variant strategy from H4.e: table uses CLIP-only; all others use 50/50 CLIP+DINOv2 average.
...
log.info("device=%s variant=%s method=%s use_clip_only=%s", device, variant, METHOD_ID, USE_CLIP_ONLY)

_preprocessor = ImagePreprocessor()

# ── DISTS setup via pyiqa ──────────────────────────────────────────────────────
log.info("Loading pyiqa DISTS metric...")
_dists_metric = pyiqa.create_metric("dists", as_loss=False).to(device)

# pyiqa DISTS expects PIL images or tensors in [0,1] range (RGB, 256x256 or any size)
...
def _flush_batch():
    ...
    if USE_CLIP_ONLY:
        # table variant: use CLIP cosine alone
...
for i, page_dir in enumerate(page_dirs):
    ...
    # Rebalanced 4-term composite: 0.3 SSIM + 0.2 (1-MSE) + 0.2 (1-LPIPS) + 0.3 (1-DISTS)
    composite = (
        0.3 * ssim_val
