#!/usr/bin/env python3
"""P1_120c3b_all_no_mask_baseline_preproc: H11.2 — extend H6.1 preprocessing to all_no_mask variant.

H11.2 hypothesis: H6.1 applied grayscale+SHARPEN+autocontrast->RGB preprocessing to the
table CLIP path and lifted table cc 0.268->0.277. variant=all_no_mask is also full-page
mixed content; applying the same preprocessing to both CLIP and DINOv2 encoders for
all_no_mask (while keeping its 50/50 fusion) may yield a similar gain.

Diff vs P1_120c3 (H6.1 baseline): ONE change only — the preprocessing gate
    was: USE_PREPROC = variant == "table"
    now: USE_PREPROC = variant in ("table", "all_no_mask")
...
METHOD_ID = "P1_120c3b_all_no_mask_baseline_preproc"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
