...
METHOD_ID = "P2_060e_docsim_h61_preproc"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
def _docsim_cosine_batch(orig_pils, recon_pils):
    """DocSim cosine — D60.e variant: H6.1 preproc applied to inputs.

    H6.1 preproc = grayscale + ImageFilter.SHARPEN + autocontrast -> RGB.
    The trained DocSim head saw RAW RGB triplets; this experiment tests
    whether the CLIP+DINOv2 features extracted from H6.1-preprocessed
    inputs still produce a useful supervised cosine after passing through
    the LoRA head.
    """
    def _encode(pils):
        preprocessed = [_baseline_clip_preprocess(p) for p in pils]
        clip_batch = torch.stack([_clip_preprocess(p.convert("RGB")) for p in preprocessed]).to(device)
        dino_batch = torch.stack([_dino_transform(p.convert("RGB")) for p in preprocessed]).to(device)
        with torch.no_grad():
