#!/usr/bin/env python3
"""D109 — Sobel edge-map cosine similarity supplement.

Computes Sobel edge maps on grayscale versions of original and reconstructed
page images, then measures cosine similarity between the flattened edge maps.
...
Uses this as an additional β_edge supplement for all/all_no_mask variants
...
scipy.ndimage.sobel for X and Y gradient, magnitude = hypot(Gx, Gy).
  ...
  all: (1-0.30-0.20-β_edge)*DocSim + 0.30*entropy + 0.20*table_cell + β_edge*sobel_edge
  all_no_mask: (1-0.40-0.20-β_edge)*DocSim + 0.40*entropy + 0.20*table_cell + β_edge*sobel_edge

β_edge=0.05 (small supplement to preserve DocSim dominance).
KEEP if > 0.4820 (D107b best). REVERT → Sobel edge supplement closed.
...
METHOD_ID = "P1_109_sobel_edge_supplement_05"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
# D109: Sobel edge-map cosine similarity supplement for all/all_no_mask
# all: (1-0.30-0.20-0.05)*DocSim + 0.30*entropy + 0.20*table_cell + 0.05*sobel_edge
# all_no_mask: (1-0.40-0.20-0.05)*DocSim + 0.40*entropy + 0.20*table_cell + 0.05*sobel_edge
BETA_SOBEL_EDGE = {
    "table": 0.0,
    "all": 0.05,
    "text": 0.0,
...
def _sobel_edge_similarity(orig_pil, recon_pil):
    """D109: cosine similarity between Sobel edge magnitude maps."""
    orig_g = np.array(orig_pil.convert("L"), dtype=np.float32)
    recon_g = np.array(recon_pil.convert("L"), dtype=np.float32)
    orig_e = np.hypot(scipy_sobel(orig_g, axis=1), scipy_sobel(orig_g, axis=0))
    recon_e = np.hypot(scipy_sobel(recon_g, axis=1), scipy_sobel(recon_g, axis=0))
    o = orig_e.ravel()
    r = recon_e.ravel()
