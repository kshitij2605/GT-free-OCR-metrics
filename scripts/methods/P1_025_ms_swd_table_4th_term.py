#!/usr/bin/env python3
"""P1_025_ms_swd_table_4th_term: Variant-conditional multi_composite with MS-SWD for table.

H2 hypothesis: MS-SWD (Multi-scale Sliced Wasserstein Distance) as 4th term in
multi_composite for variant=table only. Sliced Wasserstein distributional alignment
should capture table grid positional mismatch better than DISTS's stat averaging.

multi_composite formulas:
  variant=all (USE_DISTS=True):
    composite = 0.3*SSIM + 0.2*(1-MSE) + 0.2*(1-LPIPS) + 0.3*(1-DISTS)   # preserves H5.b
  variant=table (USE_MSSWD=True):
    composite = 0.3*SSIM + 0.2*(1-MSE) + 0.2*(1-LPIPS) + 0.3*ms_swd_sim  # NEW: MS-SWD
    where ms_swd_sim = 1 / (1 + msswd_distance)  # lower msswd -> higher similarity
  all other variants (text, formula, all_no_mask):
    composite = 0.4*SSIM + 0.3*(1-MSE) + 0.3*(1-LPIPS)  (original, unchanged)

clip_cosine logic (UNCHANGED from H4.e):
- variant=table: CLIP cosine ONLY
- all other variants: 0.5 * DINOv2_vitb14_CLS + 0.5 * OpenCLIP_ViT-B/32
"""
...
METHOD_ID = "P1_025_ms_swd_table_4th_term"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
USE_DISTS = variant == "all"          # DISTS-augmented composite only for 'all' variant (H5.b)
USE_MSSWD = variant == "table"        # MS-SWD-augmented composite only for 'table' variant (H2)
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_msswd=%s use_clip_only=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_MSSWD, USE_CLIP_ONLY,
)
...
if USE_DISTS:
    ...
    log.info("Skipping DISTS load (variant=%s)", variant)
    _dists_metric = None
    _dists_transform = None

# ── MS-SWD setup via pyiqa (only for variant=table) ──────────────────────────
if USE_MSSWD:
    import pyiqa
    log.info("Loading pyiqa MS-SWD metric (variant=table only)...")
    _msswd_metric = pyiqa.create_metric("msswd", as_loss=False).to(device)
    # msswd is lower_better=True (distance). Convert to similarity: 1 / (1 + dist)
    _msswd_transform = T.Compose([
        T.Resize((256, 256), interpolation=T.InterpolationMode.BICUBIC),
        T.ToTensor(),  # [0,1]
    ])
else:
    log.info("Skipping MS-SWD load (variant=%s)", variant)
    _msswd_metric = None
    _msswd_transform = None
...
def _msswd_similarity(orig_pil: Image.Image, recon_pil: Image.Image) -> float:
    """Compute MS-SWD similarity (higher=more similar).
    msswd returns distance (lower_better=True); convert via sim = 1/(1+dist).
    Identical images: dist=0 -> sim=1.0. Very different: dist>>0 -> sim->0.
    """
    orig_t = _msswd_transform(orig_pil.convert("RGB")).unsqueeze(0).to(device)
    recon_t = _msswd_transform(recon_pil.convert("RGB")).unsqueeze(0).to(device)
    with torch.no_grad():
        dist = _msswd_metric(orig_t, recon_t)
    dist_val = max(0.0, float(dist.item()))
...
def _flush_batch():
    ...
    if USE_CLIP_ONLY:
        # table variant: use CLIP cosine alone (H4.e strategy)
        for meta, clip_cos in zip(meta_buf, clip_sims):
            meta["clip_compare"] = {"clip_cosine": float(clip_cos)}
            results.append(meta)
    else:
        # all other variants: 50/50 CLIP + DINOv2 average (H4.e strategy)
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DISTS:
        # variant=all: DISTS-augmented rebalanced composite (preserves H5.b)
        ...
        # variant=table: MS-SWD-augmented composite (H2 hypothesis)
        msswd_sim = _msswd_similarity(orig_pil, recon_pil)
        composite = (
            0.3 * ssim_val
            + 0.2 * max(0.0, 1.0 - mse_val)
            + 0.2 * max(0.0, 1.0 - lpips_val)
