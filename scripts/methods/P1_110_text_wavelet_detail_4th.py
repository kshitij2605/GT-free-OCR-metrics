#!/usr/bin/env python3
"""P1_110_text_wavelet_detail_4th: H7.1 — DWT detail multi-scale 4th term in mc for variant=text only.

H7.1 hypothesis: Adding a multi-scale 2D-DWT detail-subband cosine similarity as a 4th additive
term in multi_composite ONLY for variant=text will lift text mc above its current ceiling of
0.3889 (frozen since H4). Wavelet detail subbands (HH/HL/LH at levels 2 and 3) capture
high-frequency stroke/edge structure that SSIM (binarised 512²) and LPIPS (alex-net) aggregate over.

multi_composite formulas:
  variant=text (USE_TEXT_DWT=True):
    composite = 0.25*SSIM + 0.25*(1-MSE) + 0.25*(1-LPIPS) + 0.25*dwt_detail_sim
  variant=all (USE_DISTS=True):
    composite = 0.3*SSIM + 0.2*(1-MSE) + 0.2*(1-LPIPS) + 0.3*(1-DISTS)
  all other variants (USE_DISTS=False, USE_TEXT_DWT=False):
    composite = 0.4*SSIM + 0.3*(1-MSE) + 0.3*(1-LPIPS)  (original, unchanged)

clip_cosine logic (unchanged from H6.1):
- variant=table: CLIP cosine ONLY, with baseline preprocessing applied before _clip_preprocess
                 (grayscale -> SHARPEN -> autocontrast -> RGB)
...
METHOD_ID = "P1_110_text_wavelet_detail_4th"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
USE_TEXT_DWT = variant == "text"      # DWT detail 4th term only for 'text' variant (H7.1)
...
# ── DWT detail similarity (only for variant=text) ────────────────────────────

def _dwt_detail_sim(orig_pil: Image.Image, recon_pil: Image.Image) -> float:
    """Multi-scale DWT detail-subband cosine similarity.
    Resize to 512x512 grayscale. db2 wavelet, levels [2,3].
    For each level, take cH+cV+cD detail subbands, flatten, L2-normalize,
    cosine = <orig, recon>. Return mean cosine across levels (clipped to [0,1])."""
    def _to_arr(p):
    ...
    cosines = []
    for level in (2, 3):
        coeffs_o = pywt.wavedec2(o, wavelet="db2", level=level)
        coeffs_r = pywt.wavedec2(r, wavelet="db2", level=level)
        # coeffs is [cA_n, (cH_n,cV_n,cD_n), ..., (cH_1,cV_1,cD_1)]; take all detail tuples
        details_o = np.concatenate([np.concatenate([d.flatten() for d in t]) for t in coeffs_o[1:]])
        details_r = np.concatenate([np.concatenate([d.flatten() for d in t]) for t in coeffs_r[1:]])
        no = np.linalg.norm(details_o)
        nr = np.linalg.norm(details_r)
        if no < 1e-8 or nr < 1e-8:
            cosines.append(0.0)
        else:
            cosines.append(float((details_o @ details_r) / (no * nr)))
    val = float(np.mean(cosines))
    # cosine in [-1,1]; map to [0,1]
    return float(np.clip(0.5 * (val + 1.0), 0.0, 1.0))
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_TEXT_DWT:
        # variant=text: DWT detail 4th term (H7.1)
        ...
        # formula, table, all_no_mask: original formula (0.4/0.3/0.3)
