#!/usr/bin/env python3
"""P2_060k — D60.k adds DocSim score as a 5th-term in mc for variant=all only.

Identical to P2_060j (the D60.j keep at 0.4093) EXCEPT: for variant=all,
the multi_composite formula gains a DocSim cosine 5th term:
    mc_all = 0.25*SSIM + 0.15*(1-MSE) + 0.15*(1-LPIPS)
             + 0.25*(1-DISTS) + 0.20*max(0, DocSim_cos)
Other variants unchanged (D60.j stack: DocSim cc for {text, all, all_no_mask};
H4.e for formula; H6.1 for table).

Hypothesis: The supervised LoRA-trained DocSim head outputs a per-page
similarity that correlates with edit-distance better than DISTS alone in
the mc 4th-term slot. If mc-DocSim lifts variant=all mc above the current
cc=0.3934 ceiling, mean rises by (mc_new - 0.3934)/5.

Predicted spearman_mean: 0.4093 (tie if mc-DocSim cannot beat cc=0.3934)
to 0.412 (if mc-DocSim lifts mc above 0.40). Most likely tie-or-marginal.
KEEP if >= 0.412. Kill switch: variant=all mc <= 0.32.
"""
...
METHOD_ID = "P2_060k_docsim_mc_4thterm_all"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
# NEW for D60.j: extend D60.b's DocSim set to include variant=text
# (text-cc DocSim = 0.4139 in post-fix audit, far above baseline mc 0.3886)
...
def _flush_batch():
    ...
    if USE_DOCSIM:
        # variant in {text, all, all_no_mask}: DocSim replaces clip_cosine
        ...
        for meta, cs in zip(meta_buf, cos_sims):
            ...
            # D60.k: for variant=all, fold DocSim cosine into mc as a 5th term
            # by recomputing the composite with the new weighting.
            if variant == "all":
                ssim_val = meta["multi_metric"]["ssim"]
                mse_val = meta["multi_metric"]["mse"]
                lpips_val = meta["multi_metric"]["lpips"]
                dists_val = meta["multi_metric"]["dists"]
                docsim_clipped = max(0.0, float(cs))
                composite_5term = (
                    0.25 * ssim_val
                ...
                meta["multi_metric"]["docsim_cos"] = float(docsim_clipped)
                meta["multi_metric"]["composite"] = float(composite_5term)
            results.append(meta)
    elif USE_CLIP_ONLY:
