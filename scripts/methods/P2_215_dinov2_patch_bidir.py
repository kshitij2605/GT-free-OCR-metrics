#!/usr/bin/env python3
"""D214 — DINOv2 patch-level soft-NN cosine for formula variant (D83 direction).

D147 (current best 0.4938): formula uses 50/50 CLIP_CLS + DINOv2_CLS.
D214: replaces DINOv2 CLS with DINOv2 patch soft-NN cosine for the formula variant.

DINOv2 ViT-B/14 at 224x224 produces 16x16=256 patch tokens (each 768-dim).
Soft-NN: for each GT patch, find the max-cosine OCR patch → mean over GT patches.
This is shift-tolerant: text that reflows slightly still matches nearby patches.

formula: 0.5*CLIP_CLS + 0.5*DINOv2_patch_softNN (same weights as D147)
text/all/all_no_mask/table: identical to D147 (no change)

KEEP if spearman_mean > 0.4938 (D147 current best).
"""
...
METHOD_ID = "P2_215_dinov2_patch_bidir"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
def _dinov2_patch_cosine_batch(orig_pils, recon_pils):
    """Symmetric soft-NN DINOv2 patch cosine: avg(GT→OCR, OCR→GT) for shift tolerance."""
    def _enc(pils):
        batch = torch.stack([_dino_transform(p.convert("RGB")) for p in pils]).to(device)
        with torch.no_grad():
            feats = _dino_model.forward_features(batch)
        return F.normalize(feats["x_norm_patchtokens"].float(), dim=-1)  # [B, N, 768]
    p_orig = _enc(orig_pils)    # [B, N, 768]
    p_recon = _enc(recon_pils)  # [B, N, 768]
    sims = torch.bmm(p_orig, p_recon.transpose(1, 2))  # [B, N, N]
    gt_to_ocr = sims.max(dim=2).values.mean(dim=1)   # [B]: each GT patch → best OCR match
    ocr_to_gt = sims.max(dim=1).values.mean(dim=1)   # [B]: each OCR patch → best GT match
    return ((gt_to_ocr + ocr_to_gt) / 2).cpu().tolist()  # [B]
...
def _flush_batch():
    ...
    if USE_DOCSIM_PAGE:
        ...
        clip_sims = _clip_cosine_batch(orig_pils_buf, recon_pils_buf)
        dino_sims = _dinov2_patch_cosine_batch(orig_pils_buf, recon_pils_buf)
