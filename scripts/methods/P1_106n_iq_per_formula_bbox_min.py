...
METHOD_ID = "P1_106n_iq_per_formula_bbox_min"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
# D106.n — per-formula-bbox IQ MIN replaces page-level IQ for formula
GAMMA_DIQA = {
    "table": 0.1,    # D106.d kept: 0.9 prod_cc + 0.1 D106
    "all": 0.0,
    "text": 0.0,
    "formula": 0.5,  # D106.n: same γ=0.50 as D106.h; IQ signal changes to per-formula-bbox MIN
    "all_no_mask": 0.0,
}[variant]
DIQA_EPS = 1e-6
...
def _diqa_per_bbox_min(orig_pil, recon_pil, bboxes, min_size=20):
    """Per-element IQ: compute IQ similarity on each bbox crop, return MIN.

    Mirrors D60.m per-cell-MIN DocSim but uses hand-crafted IQ features.
    Falls back to page-level if no valid crops.
    """
    sims = []
    for bb in bboxes:
        ...
        if oc is None or rc is None:
            continue
        if oc.width < min_size or oc.height < min_size:
            continue
        sims.append(_diqa_similarity(oc, rc))
    if not sims:
        return _diqa_similarity(orig_pil, recon_pil)
    return float(np.min(sims))


def _fuse(prod_cc, page_name, diqa_sim=0.0):
    """Convex combo of production cc + entropy + (variant=formula only) D106 IQ.

    Entropy SOURCE per-variant routed via SOURCE_ROUTING.
    For variant=formula with GAMMA_DIQA>0:
        clip_cosine = (1-alpha-gamma) * prod_cc + alpha * conf + gamma * diqa_sim
    Otherwise:
        clip_cosine = (1-alpha) * prod_cc + alpha * conf  (D224.e behavior)
    """
...
def _flush_batch():
    ...
    if USE_DOCSIM_PAGE:
        ...
        # variant=formula: 50/50 CLIP+DINOv2 fusion + D106.n per-formula-bbox IQ MIN
        clip_sims = _clip_cosine_batch(orig_pils_buf, recon_pils_buf)
        ...
        for meta, dino_cos, clip_cos, name, orig_p, recon_p in zip(
            ...
            if GAMMA_DIQA > 0.0:
                # Load per-formula-bbox for this page, fallback to page-level IQ
                formula_bbox_path = var_root / name / "ocr_formula_elements.json"
                formula_bboxes = []
                if formula_bbox_path.exists():
                    try:
                        with formula_bbox_path.open("r") as _f:
                            _elems = json.load(_f)
                        formula_bboxes = [e.get("bbox") for e in _elems if e.get("bbox")]
                    except Exception:
                        formula_bboxes = []
                diqa_sim = _diqa_per_bbox_min(orig_p, recon_p, formula_bboxes)
            else:
                diqa_sim = 0.0
