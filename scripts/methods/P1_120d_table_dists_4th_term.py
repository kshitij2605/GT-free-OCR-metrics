#!/usr/bin/env python3
"""P1_120d_table_dists_4th_term: Variant-conditional multi_composite.

H5.e hypothesis: Apply DISTS-augmented composite for variant=all AND variant=table.
All other variants (text, formula, all_no_mask) keep the original 0.4/0.3/0.3 composite.

multi_composite formulas:
  variant=all or variant=table (USE_DISTS=True):
...
Rationale: table multi_composite baseline (0.250) is below clip_cosine (0.268).
If DISTS augmentation lifts table mc above 0.268, that variant's best_over_metrics
switches from clip_cosine to multi_composite, potentially raising spearman_mean.
"""
...
METHOD_ID = "P1_120d_table_dists_4th_term"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
...
USE_DISTS = variant in {"all", "table"}   # DISTS-augmented composite for 'all' AND 'table'
...
log.info(
    "device=%s variant=%s method=%s use_dists=%s use_clip_only=%s",
    device, variant, METHOD_ID, USE_DISTS, USE_CLIP_ONLY,
)

_preprocessor = ImagePreprocessor()

# ── DISTS setup via pyiqa (only for variant=all and variant=table) ─────────────
if USE_DISTS:
    import pyiqa
    log.info("Loading pyiqa DISTS metric (variant=%s)...", variant)
...
for i, page_dir in enumerate(page_dirs):
    ...
    if USE_DISTS:
        # variant=all or variant=table: DISTS-augmented rebalanced composite
