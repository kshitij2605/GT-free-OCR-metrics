#!/usr/bin/env python3
"""P2_060s — D60.s: D60.p + 24px context padding around per-table-bbox crops.

Refinement of D60.p (best 0.4453, table cc 0.4767). Pads each table bbox
by 24 px on every side (clipped to image bounds) before the DocSim head
sees it. Hypothesis: DocSim was trained on page-level scenes which
include surrounding context; tight cell crops may starve the head of
visual cues that distinguish well-rendered vs badly rendered cells.
Adding context puts each crop closer to the training distribution.

MIN aggregation preserved (D60.p winner over MEAN/MAX). Other 4 variants
(text, formula, all, all_no_mask) byte-equal to D60.p.

Predicted spearman_mean: 0.443-0.470, most likely 0.450 (+0.005). KEEP if
>= 0.450. Kill switch: table cc <= 0.45 (regression — context dilutes).
"""
...
METHOD_ID = "P2_060s_docsim_bbox_min_padded"
SSIM_SIZE = 512
BATCH_SIZE = 16
DINO_SIZE = 224
BBOX_PAD_PX = 24  # D60.s: pad each bbox by N px on every side before crop
...
def _crop_bbox(pil: Image.Image, bbox) -> Image.Image | None:
    """Crop a bbox [x1, y1, x2, y2] padded by BBOX_PAD_PX on each side."""
    if not bbox or len(bbox) != 4:
        return None
    x1, y1, x2, y2 = map(int, bbox)
    W, H = pil.size
    x1 = max(0, min(x1 - BBOX_PAD_PX, W))
    x2 = max(0, min(x2 + BBOX_PAD_PX, W))
    y1 = max(0, min(y1 - BBOX_PAD_PX, H))
    y2 = max(0, min(y2 + BBOX_PAD_PX, H))
